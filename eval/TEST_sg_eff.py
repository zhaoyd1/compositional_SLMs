...
from eval.evaluate_lm import (R2D2GenEvaluator, R2D2GenFastEvaluator,
                              VanillaGPTEvaluator)
from utils.beam_searcher_1011 import R2D2GenFastBeamSearcher
...
class Scorer():
    ...
    def score(self, input_ids, tags):
        ...
        with open("sg_result/test_1011.txt", 'a') as file:
            file.write("model_1011_" + str(beam_size) + "\n")
            file.write("time: " + str((sum(total_time) - max(total_time) - min(total_time)) / 3) + "\t")
            file.write("ppl: " + str(total_ppl / 5) + "\n")   
        return result
