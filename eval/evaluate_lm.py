...
class VanillaGPTEvaluator:
    ...
    @torch.no_grad()
    def perplexity(self, input_ids, tag=None):
        ...
        for step in range(total_len):
            ...
            if tag != None and tag[step] == 1:
                surprisal += -torch.log(probs[0][input_ids[step]])
            ppl_sum += -torch.log(probs[0][input_ids[step]])
        if tag != None:
            if not isinstance(surprisal, torch.Tensor):
                surprisal = torch.tensor(surprisal).to(device=input_ids.device)
            return surprisal, ppl_sum
        else:
...
class R2D2GenEvaluator:
    def __init__(self, model, config, device, beam_size=1, ext_vocab: Dict[str, int]=None):
        ...
        if self.ext_vocab != None:
            print("we have ext_vocab")
        else:
            print("we dont have ext_vocab!!!!!!")
        self.device = device
    ...
    @torch.no_grad()
    def beam_search(self, input_ids, tag=None, masks=None, atom_spans=None):
        ...
        if tag != None: # (N, L)
            marginal_logp = torch.zeros(batch_size, max(seq_lens_np), device=self.device).fill_(float('-inf')) # (N, L)
        ...
        for step in range(2):
            ...
            action_log_p = F.softmax(logits, dim=-1).gather(1, tgt_ids.unsqueeze(1)).squeeze(1).log() # (N)

            beam_scores[:, 0, current_cache_offset] = beam_scores[:, 0, current_cache_offset - 1] + action_log_p

            if tag != None:
                marginal_logp[:, step] = beam_scores[:, 0, current_cache_offset]
        ...
        for batch_ids, batch_ids_cpu in zip(batch_ids_per_step[2:], batch_ids_cpu[2:]):
            ...
            shift_scores = shift_scores.squeeze(-1) # (N, B)
            reduce_scores = probs[:, :, self.reduce_id].log()  # (N, B), probability of reducing
            
            # if tag != None:
            #     joint_scores = shift_scores + current_beam_scores
            #     joint_scores.masked_fill_(~score_mask_batch[..., 0], float('-inf'))
            #     joint_scores_ = joint_scores.exp()
            #     marginal_logp.exp_().scatter_add_(dim=1, index=token_offsets_batch, src=joint_scores_).log_()
            
            # if tag != None:
            #     joint_scores = shift_scores + current_beam_scores
            #     joint_scores.masked_fill_(~score_mask_batch[..., 0], float('-inf'))
            #     # print("---------------------------------------------------------")
            #     # print("old_tmp_marginal_logp: ", marginal_logp)
            #     # print("joint_scores: ", joint_scores)
            #     # print("token_offsets_batch: ", token_offsets_batch, "token_offsets_batch_shape: ", token_offsets_batch.shape)
            #     joint_scores1 = joint_scores.exp()
            #     # print("joint_scores1: ", joint_scores1, "joint_scores1_shape: ", joint_scores1.shape)
            #     marginal_logp1 = marginal_logp.exp()
            #     # print("marginal_logp1: ", marginal_logp1)
            #     for b in range(token_offsets_batch.shape[0]):
            #         for id in range(token_offsets_batch.shape[1]):
            #             marginal_logp1[b][token_offsets_batch[b][id]] = marginal_logp1[b][token_offsets_batch[b][id]] + joint_scores1[b][id]
            #     # print("new_marginal_logp1: ", marginal_logp1)
            #     marginal_logp = marginal_logp1.log()
            #     # print("new_tmp_marginal_logp: ", marginal_logp)
            ...
            if tag != None:
                ...
                # print("joint_scores: ", joint_scores)
                # print("token_offsets_batch: ", token_offsets_batch, "token_offsets_batch_shape: ", token_offsets_batch.shape)
                temp = torch.zeros(marginal_logp.shape[0], marginal_logp.shape[1], joint_scores.shape[1]+1, device=self.device).fill_(float('-inf'))
                for b in range(token_offsets_batch.shape[0]):
                    for id in range(marginal_logp.shape[1]):
                        temp[b][id][0] = marginal_logp[b][id]
                # print("middle_temp: ", temp)
                for b in range(token_offsets_batch.shape[0]):
                    for id in range(token_offsets_batch.shape[1]):
                        temp[b][token_offsets_batch[b][id]][id+1] = joint_scores[b][id]
                # print("temp: ", temp)
                marginal_logp = temp.logsumexp(dim=-1)
                # print("new_tmp_marginal_logp: ", marginal_logp)
        ...
        if tag != None:
            ...
            return -surprisal, -beam_scores[:, :, -1].logsumexp(dim=-1) / seq_lens, states
        else:
...
class R2D2GenFastEvaluator:
    def __init__(self, model, config, device, beam_size=1, ext_vocab: Dict[str, int]=None, structonly=False):
        ...
        self.ext_vocab = ext_vocab
        if self.ext_vocab != None:
            print("fast evaluator we have ext_vocab")
        else:
            print("fast evaluator we dont have ext_vocab!!!!!!")
        self.device = device
    ...
    def beam_search(self, input_ids, tag=None, masks=None, atom_spans=None):
        ...
        for step_i in range(2):
            ...
            if tag != None:
                marginal_logp[:, step_i] = beam_scores[:, 0, current_cache_offset]
                # print("tmp_marginal_logp: ", marginal_logp)
        ...
        for batch_ids, batch_ids_cpu in zip(batch_ids_per_step[2:], batch_ids_cpu[2:]):
            ...
            # print("token_offsets_batch: ", token_offsets_batch)
            ...
            compose_scores = action_probs[:, :, ActionType.REDUCE.value]  # (N, B), probability of reducing
                        ...
                        # if tag != None:
            #     joint_scores = gen_scores + current_beam_scores
            #     joint_scores.masked_fill_(~score_mask_batch[..., 0], float('-inf'))
            #     print("---------------------------------------------------------")
            #     print("old_tmp_marginal_logp: ", marginal_logp)
            #     print("joint_scores: ", joint_scores)
            ...
            #     print("joint_scores: ", joint_scores)
            #     print("token_offsets_batch: ", token_offsets_batch, "token_offsets_batch_shape: ", token_offsets_batch.shape)
            #     new_marginal_logp = marginal_logp.exp()
            #     print("exp_marginal_logp: ", new_marginal_logp)
            #     new_joint_scores = joint_scores.exp()
            #     print("exp_joint_scores: ", new_joint_scores)
            #     marginal_logp2 = new_marginal_logp.scatter_add(dim=1, index=token_offsets_batch, src=new_joint_scores)
            #     print("marginal_logp2: ", marginal_logp2)
            #     marginal_logp = marginal_logp2.log()
            #     print("new_tmp_marginal_logp: ", marginal_logp)
