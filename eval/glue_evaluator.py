...
class DiscriminantGlueEvaluator:
    ...
    def run(self, inputs, temperature=1.0):
        inputs_device = {}
        for k, v in inputs.items():
            if v is not None and isinstance(v, torch.Tensor):
                inputs_device[k] = v.to(self.device)
                ...
                inputs_device[k] = v
        output = self.model.module(**inputs_device, temperature=temperature)
        pred = output.pred
        pred_labels = pred.argmax(dim=-1)
        gold_labels = inputs['eos_labels'] - self.min_label_id
        return pred_labels, gold_labels

    def eval(self, eval_dataloader, temperature=1.0):
        self.model.eval()
        epoch_iterator = tqdm(eval_dataloader, desc="Eval Iteration")
        predictions = []
        references = []
        with torch.no_grad():
            for _, inputs in enumerate(epoch_iterator):
                pred_labels, gold_labels = self.run(inputs, temperature=temperature)
                references.extend(gold_labels.tolist())
                predictions.extend(pred_labels.tolist())
