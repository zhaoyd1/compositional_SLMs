...
from eval.evaluate_lm import (R2D2GenEvaluator, R2D2GenFastEvaluator,
                              VanillaGPTEvaluator)
from utils.beam_searcher_0111 import R2D2GenFastBeamSearcher
...
class Scorer():
    ...
    def __init__(self, model_type, model, config, device, beam_size=300, ext_vocab=None, startofword=None):
        ...
        if model_type == "gpt":
            ...
            self.core = R2D2GenFastBeamSearcher(self.model, config, device, beam_size=beam_size, sampling=False, startofword=startofword)
        else:
    ...
    def score(self, input_ids, tags):
        ...
        for i in range(1):
            ...
            if self.model_type == "r2d2-gee":
                ...
                with torch.no_grad():
                    all_result = self.core.beam_search(target_ids=input_ids.unsqueeze(0), target_masks=torch.ones_like(input_ids.unsqueeze(0)), tag=tags.unsqueeze(0))
                    result = all_result[0]
                    state_num = all_result[2]
            elif self.model_type == "gpt":
                with torch.no_grad():
                    result = self.core.perplexity(input_ids, tags)[0]
            end_time = time.time()
            print(f"trial {i} time: ", end_time - start_time)
            loss_all = [item.score for item in all_result[1][0]]
            loss_all = torch.tensor(loss_all).to(self.device)
            ppl = torch.exp(-torch.logsumexp(loss_all, 0) / len(input_ids)).item()
            print("ppl: ", ppl)
            # print("state_num: ", state_num)
            # print("avg_length: ", sum(length_all) / len(length_all))
            # print("pointer_time: ", all_result[3])
            print("forward_step: ", all_result[2])
            print("generation_cnt: ", all_result[3])
            total_forward_step += all_result[2]
            total_gen_cnt += all_result[3]
            total_time.append(end_time - start_time)
            total_ppl += ppl
        beam_size = self.core.beam_size
        with open("sg_result/eff_0111.txt", 'a') as file:
            file.write("model_0111_" + str(beam_size) + "\n")
            file.write("ppl: " + str(total_ppl) + "\n") 
            file.write("forward_step: " + str(total_forward_step) + "\n") 
            file.write("generation_cnt: " + str(total_gen_cnt) + "\n")
        return result


class Evaluator():
    # load the whole dataset of "task_name" and compute accuracy
    def __init__(self, task_path, model_type, model, tokenizer, config, device, beam_size=300, ext_vocab=None, startofword=None):
        ...
        self.loaddata()
        self.scorer = Scorer(self.model_type, self.model, self.gpt_config, self.device, beam_size=self.beam_size, ext_vocab=self.ext_vocab, startofword=self.startofword)
    ...
    def run(self, formula, input_list):
        ...
        for item in input_list:
            ...
            item["input"] = "American Express declined to comment on the reports of plans for a board vote on a Shearson offering ."
...
if __name__ == "__main__":
    ...
    if args.pretrain_dir is not None:
        if args.model_type == 'tg':
            state_dicts = torch.load(args.pretrain_dir, map_location=lambda a, b: a)
            out_dict = {}
    ...
    if args.alltest:
        ...
        for taskname in task_list:
            ...
            evaluator = Evaluator(taskpath, args.model_type, model, tokenizer, config, device, beam_size=args.beam_size, ext_vocab=ext, startofword=startofword)
            acc = evaluator.eval()
        ...
        evaluator = Evaluator(args.task_path, args.model_type, model, tokenizer, config, device, beam_size=args.beam_size, ext_vocab=ext, startofword=startofword)
        acc = evaluator.eval()
