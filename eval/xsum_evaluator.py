...
class XSumEvaluator:
    ...
    def run(self, inputs):
        ...
        if self.model_type == "gpt":
            ...
            if self.word_sync:
                if "chunk_attn_masks" in inputs_device:
                    ...
                    inputs_device["chunk_attn_masks"], inputs_device["group_ids"], inputs_device["chunk_parses"],  max_steps=100)
                else:
                    preds = self.generator.beam_search(inputs_device["chunk_input_ids"], (inputs_device["chunk_masks"]>0).long(), inputs_device["input_ids"], inputs_device["masks"], \
                    inputs_device["group_ids"], inputs_device["chunk_parses"], max_steps=100)
            else:
                preds = self.generator.batch_random_sampling(inputs_device["chunk_input_ids"], inputs_device["chunk_masks"], inputs_device["input_ids"], inputs_device["masks"], \
                    inputs_device["group_ids"], inputs_device["chunk_parses"], max_steps=100, mode=GenerationMode.TOPK, mode_arg=2)
            ...
            if self.word_sync:
                for info in range(len(preds)):
                    preds_str = preds[info][0].to_ids()
                    preds_str = preds_str.replace('50289', '')
                    preds_str = preds_str.replace('50315', '')
                    matches = re.findall(r'\d+', preds_str)
            ...
            # print("group_ids: ", inputs_device["group_ids"])
            if self.word_sync:
                if "pointer_mask" in inputs_device:
                    preds = self.generator.beam_search(inputs_device["chunk_input_ids"], inputs_device["chunk_tgt_ids"], inputs_device["attn_masks"], inputs_device["pointer_tgt_pos"], inputs_device["pointer_mask"], max_steps=100)
                elif "attn_masks" in inputs_device:
                    preds = self.generator.beam_search(inputs_device["chunk_input_ids"], inputs_device["chunk_tgt_ids"], inputs_device["attn_masks"], max_steps=100)
                else:
                    preds = self.generator.beam_search(inputs_device["chunk_input_ids"], inputs_device["chunk_tgt_ids"], max_steps=100)
            else:
                raise Exception('current batch_random_sampling not suppport tg')
                preds = self.generator.batch_random_sampling(inputs_device["chunk_input_ids"], inputs_device["chunk_input_ids"], inputs_device["attn_masks"], max_steps=100, mode=GenerationMode.TOPK, mode_arg=2)
            # for idx in range(len(preds)):
            #     print("pred_summary: -----------------------------", preds[idx][0].to_ids())
            # for i in range(inputs_device["chunk_input_ids"].shape[0]):
            #     # print("batch_idx: ", i)
