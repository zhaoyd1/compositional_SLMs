...
auto ChunksForSequence(const T& rules, int32_array_t inputs,
  ...
  for (size_t i = 0; i < chunks.size(); ++i) {
    ...
    fstyle_int32_array_t end_of_sequence_np =
        fstyle_int32_array_t(out_shape_scalar);
    fstyle_int32_array_t token_attention_mask_np =
        NumpyFromMatrix(chunk.token_attention_mask);
    ...
    result.emplace_back(
        ...
        std::move(update_memory_from_memory_np), std::move(token_attention_mask_np));
  }
