...
StackComposeDoubleClosingNT::Transform(
  ...
  for (size_t i = 0; i < len; ++i){
    transformed_inputs[i] = inputs[i];
    transformed_input_tokens_types[i] = input_tokens_types[i];
    transformed_labels[i] = labels[i];
    transformed_label_tokens_types[i] = label_tokens_types[i];
  }
  // size_t j = 0;
  // for (size_t i = 0; i < len; ++i) {
...
StackComposeDoubleClosingNT::ComputeAttentionQuantities(
  ...
  int32_t token_cnt = 0;
  std::vector<int32_t> token_only_idx_stack; // for pop and to_attend_seq
  std::vector<int32_t> token_position_map(transformed_len, -1); // for position map, idx: position in input_seq, value: No. of all tokens
  ...
  while (cur) {
    ...
    if (cur->GetTokenType() == kPad) {
      ...
      int32_t right_child_position = right_child->GetPosition();
      int32_t left_child_position = left_child->GetPosition();
      
      if (right_child_position >= token_position_map.size() || left_child_position >= token_position_map.size()) {
        return absl::InvalidArgumentError(
            "Position is out of range.");
      }
      
      if (token_position_map[right_child_position] != -1) {
        if (token_position_map[right_child_position] != token_only_idx_stack.back()) {
          return absl::InvalidArgumentError(
              "Position right is not the last one in the stack.");
        }
        token_only_idx_stack.pop_back();
        token_position_map[right_child_position] = -1;
      }
      
      if (token_position_map[left_child_position] != -1) {
        if (token_position_map[left_child_position] != token_only_idx_stack.back()) {
          return absl::InvalidArgumentError(
              "Position left is not the last one in the stack.");
        }
        token_only_idx_stack.pop_back();
        token_position_map[left_child_position] = -1;
      }

      if (stack_set.find(right_child) != stack_set.end()) {
          // to_attend.push_back(right_child);
          stack_set.erase(right_child);
      }
      if (stack_set.find(left_child) != stack_set.end()) {
          // to_attend.push_back(left_child);
          stack_set.erase(left_child);
      }
      ...
      // to_attend = stack;
      to_attend = stack;
      token_only_idx_stack.push_back(token_cnt);
      token_position_map[cur->GetPosition()] = token_cnt;
      token_cnt++;
    ...
    // attention_info.token_only_positions = token_only_idx_stack;
    attention_info.token_only_positions.reserve(token_only_idx_stack.size());
    for (int32_t idx : token_only_idx_stack) {
      attention_info.token_only_positions.push_back(idx);
    }
...
std::vector<Chunk> StackComposeDoubleClosingNT::MakeChunks(
  ...
  for (size_t chunk_index = 0; chunk_index < num_chunks; ++chunk_index) {
    ...
    chunk.attention_indicator = Eigen::VectorXi::Zero(sequence_len_);
    chunk.attention_mask = Eigen::MatrixXi::Zero(sequence_len_, sequence_len_);
    chunk.token_attention_mask = Eigen::MatrixXi::Zero(sequence_len_, sequence_len_);
    chunk.attention_relative_position =
        Eigen::MatrixXi::Zero(sequence_len_, sequence_len_ + memory_len_);
    chunk.memory_attention_mask =
        Eigen::MatrixXi::Zero(sequence_len_, memory_len_);
    ...
    for (size_t i = 0; i < sequence_len_; ++i) {
      ...
      const std::vector<int32_t>& token_pos_to_attend =
          attention_info.token_only_positions;
      ...
      for (size_t j = 0; j < token_pos_to_attend.size(); ++j) {
        int32_t token_position_to_attend = token_pos_to_attend.at(j);
        chunk.token_attention_mask(i, token_position_to_attend - sequence_start) = 1;
      }
