...
auto ChunksForSequence(const T& rules, int32_array_t inputs,
  ...
  std::vector<std::tuple<
      ...
      fstyle_int32_array_t, fstyle_int32_array_t, fstyle_int32_array_t,
      fstyle_int32_array_t, fstyle_int32_array_t, fstyle_int32_array_t,
      fstyle_int32_array_t, fstyle_int32_array_t>>
      result;
  for (size_t i = 0; i < chunks.size(); ++i) {
    ...
    fstyle_int32_array_t pos_np = NumpyFromVector(chunk.position);
    ...
    fstyle_int32_array_t end_of_sequence_np =
        fstyle_int32_array_t(out_shape_scalar);
    fstyle_int32_array_t pointer_tgt_pos_np =
        NumpyFromVector(chunk.pointer_tgt_pos);
    fstyle_int32_array_t pointer_pos_mask_np =
        NumpyFromMatrix(chunk.pointer_pos_mask);
    ...
    result.emplace_back(
        ...
        std::move(pos_np), std::move(beginning_of_sequence_np),
        std::move(end_of_sequence_np),
        std::move(update_memory_from_sequence_np),
        std::move(update_memory_from_memory_np),
        std::move(pointer_tgt_pos_np), std::move(pointer_pos_mask_np));
  }
