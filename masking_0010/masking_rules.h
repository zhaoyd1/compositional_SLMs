...
struct Chunk {
  ...
  // Pointer to the target position for the current sequence
  Eigen::VectorXi pointer_tgt_pos;

  // Pointer candidate positions for the current sequence
  Eigen::MatrixXi pointer_pos_mask;
};
...
struct PerTokenAttentionInfo {
  ...
  int32_t pointer_tgt_pos;
  std::vector<int32_t> pointer_pos_set;
};
