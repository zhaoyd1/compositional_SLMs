...
constexpr int32_t kStartofWord = 100;
...
absl::StatusOr<std::unique_ptr<Node>> Node::FromTokenTypes(
  ...
  for (size_t i = 1; i < token_types.size(); ++i) {
    ...
    root->children_.emplace_back(std::make_unique<Node>(
        ttype, nullptr, prev, i, 0, false));
    // auto cur_node = std::make_unique<Node>(ttype, nullptr, prev, i, 0, false);
    Node* node = root->children_.back().get();
    prev->next_ = node;
    prev = node;
  }
...
void Node::SetNext(Node* next_node) { next_ = next_node; }
...
absl::StatusOr<std::vector<Chunk>> TXLCausalMasking::GetChunksForSequence(
  ...
  for (size_t chunk_index = 0; chunk_index < num_chunks; ++chunk_index) {
    ...
    chunk.position = Eigen::VectorXi::Zero(sequence_len_);  // Always 0 for TXL.
...
StackComposeDoubleClosingNT::GetChunksForSequence(
  ...
  Eigen::VectorXi pointer_tgt_pos;
  std::tie(transformed_inputs, transformed_labels,
           transformed_input_tokens_types, transformed_label_tokens_types, pointer_tgt_pos) =
  ...
  std::vector<PerTokenAttentionInfo> attention_info_vec =
    ComputeAttentionQuantities(transformed_input_tokens_types, pointer_tgt_pos).value();
...
std::tuple<Eigen::VectorXi, Eigen::VectorXi, Eigen::VectorXi, Eigen::VectorXi, Eigen::VectorXi>
StackComposeDoubleClosingNT::Transform(
  ...
  Eigen::VectorXi transformed_labels = Eigen::VectorXi::Constant(sequence_len_, -100);
  Eigen::VectorXi transformed_label_tokens_types =
      Eigen::VectorXi::Zero(sequence_len_);
  Eigen::VectorXi pointer_tgt_pos = Eigen::VectorXi::Constant(sequence_len_, -100);

  std::vector<int32_t> Sow_frontier;
  std::vector<int32_t> Sow_final_pos;

  size_t j = 0;
  size_t i = 0;
  // for (size_t i = 0; i < len; ++i) {
    // if (label_tokens_types[i] == kOpeningNonTerminal) {
    //   if (i < len )
    // }
  while (i < len) {
    int32_t tmp_i = i;
    if (label_tokens_types[i] == kOpeningNonTerminal) {
      if (input_tokens_types[i] == kClosingNonTerminal) {
        if (!Sow_final_pos.empty()) {
          pointer_tgt_pos[j - 1] = Sow_final_pos.back();
          Sow_final_pos.pop_back();
        }
        if (Sow_frontier.size() > 0 && Sow_final_pos.size() == 0) {
          Sow_final_pos.push_back(j);
          Sow_frontier.pop_back();
        }
      }
      while (tmp_i < len && label_tokens_types[tmp_i] == kOpeningNonTerminal) {
        Sow_frontier.push_back(tmp_i);
        ++tmp_i;
      }
      if (tmp_i < len && label_tokens_types[tmp_i] != kOpeningNonTerminal) {
        if (input_tokens_types[i] == kClosingNonTerminal) {
          transformed_inputs[j] = inputs[i];
          transformed_input_tokens_types[j] = kClosingNonTerminal;
          transformed_labels[j] = labels[tmp_i];
          transformed_label_tokens_types[j] = label_tokens_types[tmp_i];
          Sow_final_pos.push_back(j + 1);
          Sow_frontier.pop_back();
          j += 1;
          i = tmp_i + 1;
        }
    ...
    // if (label_tokens_types[i] == kClosingNonTerminal) {
    //   pointer_tgt_pos[j] = Sow_final_pos.back();
    //   Sow_final_pos.pop_back();
    // }
  ...
  return std::make_tuple(transformed_inputs, transformed_labels,
                         transformed_input_tokens_types,
                         transformed_label_tokens_types, pointer_tgt_pos);
}

absl::StatusOr<std::vector<PerTokenAttentionInfo>>
StackComposeDoubleClosingNT::ComputeAttentionQuantities(
    Eigen::VectorXi transformed_input_tokens_types, Eigen::VectorXi pointer_tgt_pos) const {
  ...
  // Eigen::VectorXi& token_types = transformed_input_tokens_types;
  ...
  std::vector<const Node*> transparent_seq;
  while (cur) {
    ...
    if (cur->GetTokenType() == kPad) {
      ...
      // if (stack.size() < 2) {
      //   return absl::InvalidArgumentError(
      //       "Stack is < 2 for a closing non-terminal.");
      // }

      // const Node* right_child = stack[stack.size() - 1];
      // const Node* left_child = stack[stack.size() - 2];
      if (pointer_tgt_pos[cur->GetPosition() - 1] == -100) {
        int32_t error_idx = cur->GetPosition() - 1;
        std::string error_idx_str = std::to_string(error_idx);
        std::string error_msg = "Pointer target position is -100 for a closing non-terminal at position " + error_idx_str;
        return absl::InvalidArgumentError(
            error_msg);
      }
      int32_t child_start = stack.size() - 1;
      for (int32_t child = stack.size() - 1; child >= 0; --child) {
        if (stack[child]->GetPosition() == pointer_tgt_pos[cur->GetPosition() - 1]) {
          child_start = child;
          break;
        }
      }
      
      if (child_start == stack.size() - 1 && stack[child_start]->GetPosition() != pointer_tgt_pos[cur->GetPosition() - 1]) {
        return absl::InvalidArgumentError(
            "Pointer target position is not found in the stack.");
      }

      to_attend = std::vector<const Node*>(stack.begin() + child_start, stack.end());
      stack.erase(stack.begin() + child_start, stack.end());
      
      for (const Node* nodeptr : to_attend) {
        if (stack_set.find(nodeptr) == stack_set.end()) {
          return absl::InvalidArgumentError(
              "Node is not in the stack.");
        }
        stack_set.erase(nodeptr);
      }

      stack.push_back(cur);
      stack_set.insert(cur);
      to_attend = stack;
      // transparent_seq.push_back(cur);
    ...
    attention_info.pointer_tgt_pos = pointer_tgt_pos[cur->GetPosition()];
    attention_info.position = pos;
    attention_info.pointer_pos_set.reserve(stack.size());
    bool pointer_found = false;
    for (const Node* nodeptr : stack) {
      if (nodeptr->GetTokenType() == kClosingNonTerminal || nodeptr->GetTokenType() == kStartofWord) {
        attention_info.pointer_pos_set.push_back(nodeptr->GetPosition());
        if (nodeptr->GetPosition() == pointer_tgt_pos[cur->GetPosition()]) {
          pointer_found = true;
        }
      }
    }
    if (pointer_tgt_pos[cur->GetPosition()] != -100 && !pointer_found) {
      return absl::InvalidArgumentError(
          "Pointer target position is not found.");
    }
...
std::vector<Chunk> StackComposeDoubleClosingNT::MakeChunks(
  ...
  for (size_t chunk_index = 0; chunk_index < num_chunks; ++chunk_index) {
    ...
    chunk.attention_indicator = Eigen::VectorXi::Zero(sequence_len_);
    chunk.attention_mask = Eigen::MatrixXi::Zero(sequence_len_, sequence_len_);
    chunk.pointer_pos_mask = Eigen::MatrixXi::Zero(sequence_len_, sequence_len_);
    chunk.attention_relative_position =
        Eigen::MatrixXi::Zero(sequence_len_, sequence_len_ + memory_len_);
    chunk.memory_attention_mask =
        Eigen::MatrixXi::Zero(sequence_len_, memory_len_);
    chunk.memory_padding_mask = Eigen::VectorXi::Zero(memory_len_);
    chunk.memory_position = Eigen::VectorXi::Zero(memory_len_);
    chunk.position = Eigen::VectorXi::Zero(sequence_len_);
    chunk.pointer_tgt_pos = Eigen::VectorXi::Constant(sequence_len_, -100);
    ...
    for (size_t i = 0; i < sequence_len_; ++i) {
      ...
      const std::vector<int32_t>& pointer_pos_set =
          attention_info.pointer_pos_set;
      const int32_t attention_indicator = attention_info.indicator;
      const int32_t pos = attention_info.position;
      if (chunk.inputs[i] > 0) {
        last_nonzero_index = sequence_index;
      }
      chunk.attention_indicator[i] = attention_indicator;
      chunk.position[i] = pos;
      chunk.pointer_tgt_pos[i] = attention_info.pointer_tgt_pos;
      if (!gather_into_new_memory_ && (chunk.inputs[i] > 0)) {
        memory_padding_mask_[i] = 1;
        chunk.update_memory_from_sequence(i, i) = 1;
        memory_position_from_index_[i] = sequence_index;
      }
      for(size_t j = 0; j < pointer_pos_set.size(); ++j) {
        chunk.pointer_pos_mask(i, pointer_pos_set[j] - sequence_start) = 1;
      }
