...
StackComposeDoubleClosingNT::Transform(
  ...
  Eigen::VectorXi transformed_label_tokens_types =
      Eigen::VectorXi::Zero(transformed_len);
  Eigen::VectorXi pointer_tgt_pos = Eigen::VectorXi::Zero(transformed_len);

  for (size_t i = 0; i < transformed_len; ++i) {
    pointer_tgt_pos[i] = -100;
  }
    ...
    if (label_tokens_types[i] == kOpeningNonTerminal) {
      ...
      if (tmp_i < len && label_tokens_types[tmp_i] != kOpeningNonTerminal) {
        if (input_tokens_types[i] == kClosingNonTerminal) {
          ...
          transformed_labels[j] = -100;
          transformed_labels[j + 1] = labels[tmp_i];
          transformed_label_tokens_types[j] = kPad;
          transformed_label_tokens_types[j + 1] = label_tokens_types[tmp_i];
          Sow_final_pos.push_back(j + 2);
          Sow_frontier.pop_back();
          j += 2;
          i = tmp_i + 1;
        }
...
std::vector<Chunk> StackComposeDoubleClosingNT::MakeChunks(
  ...
  for (size_t chunk_index = 0; chunk_index < num_chunks; ++chunk_index) {
    ...
    chunk.pointer_tgt_pos = Eigen::VectorXi::Zero(sequence_len_);
    chunk.pointer_tgt_pos = Eigen::VectorXi::Zero(sequence_len_);
    for (size_t i = 0; i < sequence_len_; ++i) {
      chunk.pointer_tgt_pos[i] = -100;
    }
    chunk.update_memory_from_sequence =
        Eigen::MatrixXi::Zero(sequence_len_, memory_len_);
    chunk.update_memory_from_memory =
