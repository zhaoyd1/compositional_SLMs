...
class FastGenerativeR2D2(nn.Module):
    ...
    def forward(self, chunk_input_ids= None, chunk_masks=None, input_ids=None, masks=None, eos_labels=None, group_ids=None, 
        ...
        if self.enable_gpt:
            ...
            reduce_ids = torch.ones((ldr_repr.shape[0], ldr_repr.shape[1]), 
                                    dtype=torch.long,
                                    device=chunk_input_ids.device).fill_(self.r2d2.reduce_id)
            gpt_input = self.embeddings(reduce_ids)
        ...
        return R2D2GenOutput(struct_loss=lm_loss + l_height, 
                             non_struct_loss=gpt_loss + 2.0 * action_loss + parser_loss,
                             logits=logits,
