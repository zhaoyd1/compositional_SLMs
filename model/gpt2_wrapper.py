...
class GPT2Wrapper(nn.Module):
    ...
    def forward(self, chunk_input_ids=None, chunk_masks=None, input_ids=None, masks=None, group_ids=None, 
                max_input_len=0, atom_spans=None, enable_gpt=True, coeff=1.0, eos_labels=None, tg_inference=False, **kwargs):
        if eos_labels is None:
            ...
            # print("gpt_input_ids: ", gpt_input_ids, "tgt_ids: ", tgt_ids)
            # print("gpt_input_ids: ", len(gpt_input_ids[0]), "tgt_ids: ", len(tgt_ids[0]))
            # print("gpt_input_ids: ", (gpt_input_ids[0]).max(), "tgt_ids: ", (tgt_ids[0]).max())
            # exit()
            try:
                result = self.gpt(input_ids=gpt_input_ids, labels=tgt_ids, return_dict=True, tg_inference=tg_inference)
            except:
                
                exit()
            return R2D2GenOutput(non_struct_loss=result.loss, struct_loss = 0, logits=result.logits, hidden_states=result.hidden_states[-1], 
