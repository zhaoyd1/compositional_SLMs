...
from model.n_ary_gen import N_ARY_GEN
...
def create_model(model_type, r2d2_config_path, gpt_config_path, fix_embeddings=False, gradient_checkpoint=False, opening_included=False):
    if model_type == 'r2d2':
        gpt_config = AutoConfig.from_pretrained(gpt_config_path)
        vocab_size = gpt_config.vocab_size
        r2d2_config = AutoConfig.from_pretrained(r2d2_config_path)
    ...
    #                                    gradient_checkpoint=gradient_checkpoint)
        ...
        r2d2_input_dim = r2d2.input_dim
        gpt_input_dim = gpt_config.n_embd
        generative_r2d2 = FastGenerativeR2D2(r2d2, action_transformers, None, vocab_size, r2d2_input_dim,\
                                             gpt_input_dim, ext_vocab_size=r2d2_config.ext_vocab_size)
        return generative_r2d2
    
    elif model_type == 'n-ary-gen':
        r2d2_config = AutoConfig.from_pretrained(r2d2_config_path)
        N_ary_compose_fn = N_ARY_InsideEncoder(r2d2_config)
        composed_input_dim = N_ary_compose_fn.input_dim
        gpt_config = AutoConfig.from_pretrained(gpt_config_path)
        vocab_size = gpt_config.vocab_size
        gpt_config.n_layer = gpt_config.action_layer_num
        action_transformers = GPT2Model(gpt_config, no_embedding=True, no_layer_norm=True)
        action_transformers.gradient_checkpointing = gradient_checkpoint
        gpt_input_dim = gpt_config.n_embd
        n_ary_gen = N_ARY_GEN(N_ary_compose_fn, action_transformers, None, vocab_size, composed_input_dim, gpt_input_dim, ext_vocab_size=r2d2_config.ext_vocab_size, opening_included=opening_included)
        return n_ary_gen
        ...
        from model.transformer_grammar import TG
        gpt_config = AutoConfig.from_pretrained(gpt_config_path)
        tg = TG(gpt_config)
        tg.gpt.transformer.gradient_checkpointing = gradient_checkpoint
        return tg
...
def glue_create_model(model_type, r2d2_config_path, gpt_config_path, fix_embeddings=False, gradient_checkpoint=False, finetune_class_num=-1):
    if model_type in ['r2d2-gen-fast', 'r2d2-gen-fast-struct'] and finetune_class_num != -1:   # discriminant r2d2-gen-fast
        if model_type == "r2d2-gen-fast":
            from model.generative_r2d2_fast import FastGenerativeR2D2_discriminant_glue
        elif model_type == 'r2d2-gen-fast-struct':
            from model.generative_r2d2_fast_abl_structonly import FastGenerativeR2D2_discriminant_glue
        gpt_config = AutoConfig.from_pretrained(gpt_config_path)
        vocab_size = gpt_config.vocab_size
        total_layer = gpt_config.n_layer
        gpt_config.n_layer = gpt_config.action_layer_num
