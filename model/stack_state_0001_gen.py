...
class State:
    ...
    def act(self, action: ActionType, token_id = -1):
        ...
        if action == ActionType.SHIFT:
            ...
            next_state.composed_position = self.composed_position | set()
            ...
            next_state.composed_position = self.composed_position | set([self.prev_shift.current_step, self.current_step])
            next_state.token_id = 50289
            next_state.current_seq = self.current_seq + [next_state.token_id]
            # TODO: for n-ary, it should record all states (by Node)
...
class BeamContext:
    ...
    def init_history_kv(self, action_key_values, chunk_attn_masks, seq_lens, group_ids):
        ...
        for batch_i in range(N):
            self.action_masks[batch_i, 0, :action_len_bucket[batch_i]] = chunk_attn_masks[batch_i, action_len_bucket[batch_i] - 1, :action_len_bucket[batch_i]]
    ...
    def prepare_gpt_input(self, states_batch: List[List[State]], sync_steps: List[int]):
        ...
        for batch_i, states in enumerate(states_batch):
            for state_i, state in enumerate(states):
                ...
                if state.token_offset == sync_steps[batch_i] and not state.is_finished:
                    ...
                    offset = torch.tensor(list(state.composed_position), dtype=torch.long, device=self.device) + self.action_kv_history_len
        ...
        if len(gpt_input_indices) > 0:
            ...
            #     print(states_batch[kv_indices[i,0]][kv_indices[i,1]].composed_position)
            # print(position_ids)
            # print(action_masks)
            # breakpoint()
