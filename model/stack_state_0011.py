...
class State:
    ...
    def is_finished(self):
        if self.token_offset == self.total_len and (len(self.stack) == 1 or self.all_closing_nt_cnt == self.total_len):
    ...
    def act(self, action: ActionType, token_id = -1, pointer_tgt_id = -1):
        ...
        if action == ActionType.SHIFT:
            ...
            if self.invalid_state or len(self.stack) < 1 \
...
class BeamContext:
    def __init__(self, model, batch_size, max_beam_size, max_input_len, compose_dim, config, device, 
        ...
        self.compose_cache = torch.full((batch_size, max_beam_size, max_input_len * 2 + 1, compose_dim), fill_value=0.0, device=device)
        ...
        self.action_key_values = [(torch.full((batch_size, max_beam_size, self.head_num, 2 + 2 * max_input_len + action_kv_history_len, self.head_dim), 
                                              fill_value=INIT_VAL, device=device),
                                   torch.full((batch_size, max_beam_size, self.head_num, 2 + 2 * max_input_len + action_kv_history_len, self.head_dim), 
                                              fill_value=INIT_VAL, device=device)) for _ in range(self.action_layer_num)]
        self.action_masks = torch.full((batch_size, max_beam_size, 2 * max_input_len + action_kv_history_len + 3), fill_value=False, dtype=torch.bool, device=device)
        ...
        self.pointer_network_cache = torch.full((batch_size, max_beam_size, 2 + 2 * max_input_len + action_kv_history_len, config.n_embd // 6), fill_value=INIT_VAL, device=device) # the pointer network cache
    ...
    def prepare_gpt_input(self, states_batch: List[List[State]], sync_steps: List[int]):
        ...
        for batch_i, states in enumerate(states_batch):
            for state_i, state in enumerate(states):
                ...
                if state.token_offset == sync_steps[batch_i] and not state.is_finished:
                    ...
                    cur_pointer_masks = torch.full((action_mask_len,), False, dtype=torch.bool, device=self.device)
                    if state.current_step in state.pointer_candidate_pos:
                        ...
                        true_pointer_candidate_pos = true_pointer_candidate_pos | set({-1}) 
                    else:
                        true_pointer_candidate_pos = state.pointer_candidate_pos
                    offset = torch.tensor(list(true_pointer_candidate_pos), dtype=torch.long, device=self.device)
        ...
        if len(gpt_input_indices) > 0:
            ...
            pointer_network_cache = self.pointer_network_cache[kv_indices[:, 0], kv_indices[:, 1], :, :] # (?, L, dim)
            action_masks = self.action_masks[kv_indices[:, 0], kv_indices[:, 1], :]  #(?, L)
            action_masks = torch.logical_and(action_masks, tg_masks)
