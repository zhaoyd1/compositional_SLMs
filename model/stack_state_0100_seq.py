...
class State:
    ...
    def act(self, action: ActionType, token_id = -1):
        ...
        if action == ActionType.SHIFT:
            ...
            if self.invalid_state or len(self.stack) < 2 \
...
class BeamContext:
    def __init__(self, model, batch_size, max_beam_size, max_input_len, config, device, 
        ...
        self.action_key_values = [(torch.full((batch_size, max_beam_size, self.head_num, 3 + 2 * max_input_len + action_kv_history_len, self.head_dim), 
                                              fill_value=INIT_VAL, device=device),
                                   torch.full((batch_size, max_beam_size, self.head_num, 3 + 2 * max_input_len + action_kv_history_len, self.head_dim), 
                                              fill_value=INIT_VAL, device=device)) for _ in range(self.action_layer_num)]
        self.action_masks = torch.full((batch_size, max_beam_size, 2 * max_input_len + action_kv_history_len + 4), fill_value=False, dtype=torch.bool, device=device)
