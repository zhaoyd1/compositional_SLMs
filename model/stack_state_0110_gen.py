...
class State:
    def __init__(self, prev_state = None) -> None:
        ...
        self.score = 0
        self.beam_id = -1
        self.startofword = prev_state.startofword if prev_state is not None else tuple() # the vocabulary id of the start of word, do not change
        self.pointer_candidate_pos = set() # the candidate positions for pointer network
        self.batch_idx = -1
        ...
        self.pointer_pos_seq = prev_state.pointer_pos_seq[:] if prev_state is not None else [] # the history of pointer positions, only for debug
        self.cur_limited_pointer_candidate_pos = -100 # the position of the current limited pointer candidate, for avoiding unary chain, reduce2 cannot point to the last element of the stack (composed node)
    ...
    def is_finished(self):
        if self.token_offset == self.total_len:
    ...
    def action_masks(self, atom_spans=None):
        ...
        if self.all_closing_nt_cnt < self.total_len and len(self.stack) >= 1:
            reduce_valid = True
    ...
    def act(self, action: ActionType, token_id = -1, pointer_tgt_id = -1):
        ...
        if action == ActionType.SHIFT:
            ...
            if next_state.startofword[token_id]:
                next_state.pointer_candidate_pos = self.pointer_candidate_pos | set({next_state.current_step})
            else:
                next_state.pointer_candidate_pos = self.pointer_candidate_pos | next_state.pointer_candidate_pos
            return next_state
        
        elif action == ActionType.REDUCE:
            # assert self.stack_depth >= 2
            if self.invalid_state or self.current_action == ActionType.REDUCE or len(self.stack) < 1 \
            or self.all_closing_nt_cnt >= self.total_len:
            ...
            if pointer_tgt_id >= self.max_step:
                pointer_tgt_id = self.current_step
            try:
                pointer_tgt_stack_pos = self.stack.index(pointer_tgt_id)
            except:
                next_state.invalid_state = True
                next_state.token_id = token_id
                next_state.input_offset = self.total_len - 1
                # print(self.current_seq)
                # print(pointer_tgt_id)
                # print(self.stack)
                # print("hhhhh")
                return next_state
            next_state.current_seq = self.current_seq + [token_id]
            ...
            #     next_state.input_offset = self.total_len - 1
            #     return next_state    
            next_state.pointer_pos_seq = self.pointer_pos_seq + [pointer_tgt_id]
            next_state.stack = self.stack[:pointer_tgt_stack_pos] + [next_state.current_step]
            ...
            next_state.cur_composition_positions = set(self.stack[pointer_tgt_stack_pos:])
            next_state.pointer_candidate_pos = self.pointer_candidate_pos - next_state.cur_composition_positions # pop the positions of composed nodes
            next_state.pointer_candidate_pos = next_state.pointer_candidate_pos | {next_state.current_step} # add the position of the new node
            next_state.composed_positions = set(self.stack[pointer_tgt_stack_pos:]) | next_state.composed_positions # the positions of composed nodes
            # next_state.cur_limited_pointer_candidate_pos = -1 # current_step is the position of the composed node, and in the pointer_logits, current_step is of index -1
            next_state.token_id = token_id
            return next_state
            ...
            next_state.current_seq = self.current_seq + [token_id]
            next_state.pointer_candidate_pos = self.pointer_candidate_pos | next_state.pointer_candidate_pos
            # next_state.cur_limited_pointer_candidate_pos = self.current_step # current_step is the position of the composed node
            return next_state
        else:
...
class BeamContext:
    def __init__(self, model, batch_size, max_beam_size, max_input_len, config, device, 
        ...
        self.pointer_network_cache = torch.full((batch_size, max_beam_size, 4 + 3 * max_input_len + action_kv_history_len, config.n_embd // 6), 
                                                fill_value=INIT_VAL, device=device) # the pointer network cache
    def init_history_kv(self, action_key_values, pointer_network_cache, seq_lens, attn_masks):
        ...
        self.pointer_network_cache[:, 0, :self.action_kv_history_len, :] = pointer_network_cache
        # set masks
        # N = max(group_ids) + 1
        ...
        # breakpoint()
        action_len_bucket = seq_lens
        N = len(action_len_bucket)
        self.prefix_token_lens = [l.item() - 1 for l in action_len_bucket] 
        # attn_masks: (N, L, L)
        for batch_i in range(N):
            self.action_masks[batch_i, 0, :action_len_bucket[batch_i]] = attn_masks[batch_i, action_len_bucket[batch_i] - 1, :action_len_bucket[batch_i]]
            self.action_masks[batch_i, 0, action_len_bucket[batch_i] - 1] = False # it should be a reduce2 action
    ...
    def update(self, states_batch: List[List[State]], sync_steps: List[int],  
               last_action_kv, last_pointer_network_cache):
        ...
        L = self.pointer_network_cache.shape[-2]
        self.pointer_network_cache = self.pointer_network_cache.gather(dim=1, index=org_beam_ids.unsqueeze(2).unsqueeze(3)\
                                                                .repeat(1, 1, L, self.pointer_network_cache.shape[-1]))
        ...
        if action_kv_indices.shape[0] == 0:
            ...
            if last_pointer_network_cache is not None:
                assert torch.all(self.pointer_network_cache[action_kv_indices[:, 0], action_kv_indices[:,1], action_kv_indices[:,2] - 1, :] == INIT_VAL)
                self.pointer_network_cache[action_kv_indices[:, 0], action_kv_indices[:,1], action_kv_indices[:,2] - 1, :] = \
                    last_pointer_network_cache.index_select(dim=0, index=action_kv_reorder_indices).squeeze(-2)
    ...
    def prepare_gpt_input(self, states_batch: List[List[State]], sync_steps: List[int]):
        ...
        for batch_i, states in enumerate(states_batch):
            for state_i, state in enumerate(states):
                ...
                if state.token_offset == sync_steps[batch_i] and not state.is_finished:
                    ...
                    cur_pointer_masks = torch.full((action_mask_len,), fill_value=False, dtype=torch.bool, device=self.device)
                    if state.current_step in state.pointer_candidate_pos:
                        true_pointer_candidate_pos = state.pointer_candidate_pos - set({state.current_step})
                        true_pointer_candidate_pos = true_pointer_candidate_pos | set({-1 - self.action_kv_history_len}) # -1 is the current action
                    else:
                        true_pointer_candidate_pos = state.pointer_candidate_pos
                    offset = torch.tensor(list(true_pointer_candidate_pos), dtype=torch.long, device=self.device) + self.action_kv_history_len
                    cur_pointer_masks[offset] = True
                    pointer_masks.append(cur_pointer_masks)
        ...
        if len(gpt_input_indices) > 0:
            ...
            pointer_masks = torch.stack(pointer_masks, dim=0)
            assert tg_masks.shape == (len(gpt_input_indices), action_mask_len)
            assert pointer_masks.shape == (len(gpt_input_indices), action_mask_len)
        ...
        if len(gpt_input_indices) > 0:
            ...
            pointer_network_cache = self.pointer_network_cache[gpt_input_indices[:, 0], gpt_input_indices[:, 1], :, :] # (?, L, dim)
            # TODO: add additional mask by current state
            action_masks = self.action_masks[kv_indices[:, 0], kv_indices[:, 1], :]  #(?, L + 1)
            ...
            #     print(states_batch[0][i].pointer_pos_seq)
            #     print(states_batch[0][i].pointer_candidate_pos)
            ...
            #     print(states_batch[kv_indices[i,0]][kv_indices[i,1]].current_step)
            #     print(states_batch[kv_indices[i,0]][kv_indices[i,1]].token_offset)
            #     print(states_batch[kv_indices[i,0]][kv_indices[i,1]].all_closing_nt_cnt)
            #     print(states_batch[kv_indices[i,0]][kv_indices[i,1]].pointer_pos_seq)
            #     print(states_batch[kv_indices[i,0]][kv_indices[i,1]].pointer_candidate_pos)
            #     print(states_batch[kv_indices[i,0]][kv_indices[i,1]].stack)
            #     print(states_batch[kv_indices[i,0]][kv_indices[i,1]].composed_positions)
            # breakpoint()
        # print(action_masks)
            return gpt_inputs, position_ids, action_kv, pointer_network_cache, action_masks, pointer_masks
        else:
