...
class State:
    def __init__(self, prev_state = None) -> None:
        ...
        self.stack = prev_state.stack[:] if prev_state is not None else [] # the stack of node depth
        self.stack_depth = prev_state.stack_depth if prev_state is not None else 0 # current stack depth
        ...
        self.hidden_pos = prev_state.hidden_pos if prev_state is not None else 0
        # TODO: modify the max_succ_open_nt to 5 for xsum
        self.max_succ_open_nt = 5 # the max number of successive open nt
        self.all_open_nt_cnt = prev_state.all_open_nt_cnt if prev_state is not None else 0 # the number of all open nt
    ...
    def action_masks(self, atom_spans=None):
        ...
        if self.token_offset < self.total_len and self.uncomposed_open_nt == 0 and len(self.stack) > 0:
            self.invalid_state = True # all composed before the end of the sequence
        ...
        if len(self.stack) >= 2 and self.stack[-1] == self.stack[-2]:
            if self.uncomposed_open_nt == 0:
                print(self.current_seq)
                print(self.stack)
                print(self.stack_depth)
                exit()
            reduce_valid = True # immediately reduce as there are two nodes of the same depth
        ...
        and self.all_open_nt_cnt < self.total_len - 1: # do not allow over n - 1 opening nts
            open_valid = True
    ...
    def act(self, action: ActionType, token_id = -1):
        ...
        if action == ActionType.SHIFT:
            ...
            if self.stack_depth > 0:
                next_state.stack = self.stack[:] + [self.stack_depth]
            next_state.composed_positions = self.composed_positions | next_state.composed_positions
            ...
            next_state.pos = self.hidden_pos + 1
            next_state.hidden_pos = self.hidden_pos + 1
            return next_state
        
        elif action == ActionType.REDUCE:
            # assert self.stack_depth >= 2
            if self.invalid_state or self.current_action == ActionType.OPEN or self.uncomposed_open_nt <= 0 or \
            len(self.stack) < 2 or self.stack[-1] != self.stack[-2]:
                ...
                next_state.token_id = token_id
                return next_state
            assert len(self.stack) >= 2 and self.stack[-1] == self.stack[-2]
            next_state = State(self)
            next_state.stack = self.stack[:-2] # pop the top two nodes
            next_state.stack_depth = self.stack_depth - 1
            next_state.stack = next_state.stack + [self.stack_depth - 1] # push the new node
            ...
            next_state.current_seq = self.current_seq + [token_id]
            next_state.token_id = token_id
            next_state.pos = self.hidden_pos
            return next_state
...
class BeamContext:
    def __init__(self, model, batch_size, max_beam_size, max_input_len, compose_dim, config, device, 
        ...
        self.lparen_embedding = model.lparen_embedding
        ...
        self.action_key_values = [(torch.full((batch_size, max_beam_size, self.head_num, 2 + 3 * max_input_len + action_kv_history_len, self.head_dim), 
                                              fill_value=INIT_VAL, device=device),
                                   torch.full((batch_size, max_beam_size, self.head_num, 2 + 3 * max_input_len + action_kv_history_len, self.head_dim), 
                                              fill_value=INIT_VAL, device=device)) for _ in range(self.action_layer_num)]
        self.action_masks = torch.full((batch_size, max_beam_size, 3 * max_input_len + action_kv_history_len + 3), fill_value=False, dtype=torch.bool, device=device)
    ...
    def prepare_compositions(self, states_batch, beam_context):
        ...
        for batch_i, states in enumerate(states_batch):
            for state in states:
                if state.current_action == ActionType.REDUCE and not state.is_finished:
                    assert len(state.cur_composition_positions) == 2 # binary tree
        ...
        if len(composition_indices) > 0:
            ...
            comp_repr = self.compose_cache[composition_indices[:, 0], composition_indices[:, 1], composition_indices[:, 2], :] # (? * 2, dim)
            comp_repr = comp_repr.view(-1, 2, self.compose_dim) # (?, 2, dim)
            return comp_repr # (?, 2, dim)
        else:
            return None, None
        ...
        for batch_i, states in enumerate(states_batch):
            for state_i, state in enumerate(states):
                ...
                if state.token_offset == sync_steps[batch_i] and not state.is_finished:
                    ...
                    if state.current_action == ActionType.OPEN:
