        ...
        for batch_i, states in enumerate(states_batch):
            for state_i, state in enumerate(states):
                ...
                if state.token_offset == sync_steps[batch_i] and not state.is_finished:
                    ...
                    cur_mask = torch.full((action_mask_len,), True, dtype=torch.bool, device=self.device)
                    offset = torch.tensor(list(state.composed_positions), dtype=torch.long, device=self.device)
