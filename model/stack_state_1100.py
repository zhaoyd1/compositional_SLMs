...
class State:
    ...
    def action_masks(self, atom_spans=None):
        ...
        if self.current_action == ActionType.REDUCE:
            reduce2_valid = True
            return [shift_valid, reduce_valid, open_valid, reduce2_valid]
    ...
    def act(self, action: ActionType, token_id = -1):
        ...
        if action == ActionType.OPEN:
            if self.prev_succ_open_nt >= self.max_succ_open_nt or self.invalid_state or \
        ...
        if action == ActionType.SHIFT:
            ...
            if self.invalid_state or self.current_action == ActionType.REDUCE or \
            self.current_action == ActionType.OPEN or self.uncomposed_open_nt <= 0:
            ...
            next_state.cur_composition_positions = set(range(matched_open_nt_pos, next_state.current_step)) - next_state.composed_positions # the positions of composed nodes to reduce, not contain current position of next state
...
class BeamContext:
    def __init__(self, model, batch_size, max_beam_size, max_input_len, config, device, 
        ...
        self.action_key_values = [(torch.full((batch_size, max_beam_size, self.head_num, 4 + 4 * max_input_len + action_kv_history_len, self.head_dim), 
                                              fill_value=INIT_VAL, device=device),
                                   torch.full((batch_size, max_beam_size, self.head_num, 4 + 4 * max_input_len + action_kv_history_len, self.head_dim), 
                                              fill_value=INIT_VAL, device=device)) for _ in range(self.action_layer_num)]
        self.action_masks = torch.full((batch_size, max_beam_size, 4 * max_input_len + action_kv_history_len + 5), fill_value=False, dtype=torch.bool, device=device)
    ...
    def prepare_gpt_input(self, states_batch: List[List[State]], sync_steps: List[int]):
        ...
        if len(gpt_input_indices) > 0:
            ...
            #     print(states_batch[0][i].all_open_nt_cnt)
            #     print(states_batch[0][i].prev_succ_open_nt)
            #     print(states_batch[0][i].stack_depth)
            #     print(states_batch[0][i].stack)
            #     print(states_batch[0][i].composed_positions)
            #     print(states_batch[0][i].cur_composition_positions)
            #     print(states_batch[0][i].open_nt_pos_seq)
