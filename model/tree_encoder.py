...
class TreeEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward, dropout, max_role_count, 
        ...
        if binary:
            ...
            self.position_embedding = PositionalEmbedding(d_model) # default batch_first=True
    ...
    def forward(self, src, src_mask=None, pos_ids=None):
        ...
        if self.binarized:
            position_embedding = self.position_embedding(pos_ids) # (None, l, dim)
        else:
            position_embedding = self.position_embedding(pos_ids.squeeze(0)) # (None, l, dim)
...
class N_ARY_InsideEncoder(nn.Module):
    ...
    # @property
    def pos_ids(self, n_comps):
        # if self._pos_ids is None:
        #     self._pos_ids = torch.arange(2).to(self.device)
        # return self._pos_ids
        return torch.arange(n_comps).to(self.device)

    def forward(self, src, pad_mask=None, span_embeds=None):
        """
        :param src: [batch_size, n_comps, dim]
        :param span_embeds: [batch_size, dim]
        :pad_mask: [batch_size, n_comps]
        :return:
        """
        dim = src.shape[-1]
        org_shape = src.shape  
        ...
        for mod in self.layers:
            output = mod(output, pos_ids=self.pos_ids(n_comps).unsqueeze(0)) # (batch_size, n_comps, dim)
        
        # add mask to output
        if pad_mask is not None:
            output = output * pad_mask.unsqueeze(-1)

        if span_embeds is not None:
            output = output.sum(dim=1).view(*org_shape[:-2], dim) + span_embeds.unsqueeze(1)
        else:
            output = output.sum(dim=1).view(*org_shape[:-2], dim) # (batch_size, dim)
