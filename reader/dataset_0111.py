...
class GPT2Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.tg:
            ...
            pointer_tgt_pos = chunk.pointer_tgt_pos[:length + 1]
            pointer_mask = chunk.pointer_mask[:length + 1, :length + 1]
            ...
            return {"src": np.array(src_p), "tgt": np.array(tgt_p), "mask": np.array(mask), "relpos": np.array(relpos), 
                    "pointer_tgt_pos": np.array(pointer_tgt_pos), "pointer_mask": np.array(pointer_mask)}
