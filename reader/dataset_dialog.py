...
class XSumDataset(data.Dataset):
        ...
        for input_example in tqdm(self.input_examples):
            ...
            for document_sent in input_example["document"]:
                ...
                if document_sent == "__eou__":
                    ...
                    ids = self._tokenizer.encode(document_sent)
                cur_len = len(ids)
                seq_idx += 1
                if sentence_splits[-1] + cur_len <= self.document_threshold:
                    text = text + ids
    ...
    def __getitem__(self, idx):
        ...
        # print(summarytext)
        current_item = {"src": text, "summary": summarytext}
        return current_item
