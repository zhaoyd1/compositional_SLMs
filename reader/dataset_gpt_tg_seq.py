...
class GPT2Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.tg:
            tokens, splits, _ = self.getidx(data_idx)
            num_tokens = splits[-1]
            one_sent_length = splits[0]
            ...
            # if tokens_to_strip > 0:
                ...
                transformed_length = splits[-1] - splits[start_idx]
                one_sent_length = splits[start_idx + 1] - splits[start_idx]
                
                while transformed_length < self.max_seq_len:
                    data_idx = (data_idx + 1) % self.ds_len
                    new_tokens, splits, _ = self.getidx(data_idx)
                    new_len = splits[-1]
            ...
            # print(sum([1 for item in info_tuple["labels_ttypes"] if item == 3]))
            # print(sum([1 for item in info_tuple["labels_ttypes"] if item == 4]))
            ...
            return {"src": np.array(src_p), "tgt": np.array(tgt_p)}
        else:
