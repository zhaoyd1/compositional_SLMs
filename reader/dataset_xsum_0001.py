...
class XSumDataset(data.Dataset):
    ...
    def __init__(self, data_dir, mode, tokenizer, eos_id=50256, document_threshold=900, summary_threshold=100, seperator=" ", ranges=None, maskrules=None, **kwargs):
    ...
    def xsum_load_dataset(self, data_path_or_dir, **kwargs) -> List[Dict]:
        ...
        if self.mode == "train":
            self.input_examples = self.get_examples("train/train_clean.json")
            input_id_file = "train/xsum_train_0000.txt"
            input_seq_file = "train/xsum_train_0001.txt"
            input_parse_file = "train/xsum_train_0001_parses.txt"
        elif self.mode == "test":
            self.input_examples = self.get_examples("test/test_clean.json")
            input_id_file = "test/xsum_test_0000.txt"
            input_seq_file = "test/xsum_test_0001.txt"
            input_parse_file = "test/xsum_test_0001_parses.txt"
        

        self.input_ids = self.get_input_ids(input_id_file)
        self.input_parses = self.get_input_parses(input_parse_file)
        self.input_seq = self.get_input_ids(input_seq_file)
        ...
        for input_example in tqdm(self.input_examples):
            ...
            text_seq = text_seq + [self._tokenizer.convert_tokens_to_ids("Summary"), self._tokenizer.convert_tokens_to_ids(":"), self._tokenizer.convert_tokens_to_ids("X)")]
            ...
            for summary_sent in input_example["summary"]:
                ...
                if self.mode == "train" or self.mode == "train_tiny":
                    text = text + ids
                    text_seq = text_seq + self.input_seq[sent_idx]
                    sentence_splits.append(len(text))
                    sentence_parses.append(parses)
                    assert len(ids) == len(parses) + 1
                sent_idx += 1
            ...
            if self.mode == "train" or self.mode == "train_tiny":
                ...
                text_seq = text_seq + [self._eos_id]
                sentence_splits.append(len(text))
                sentence_parses.append([])
            text = np.array(text)
            text_seq = np.array(text_seq)
            assert len(text_seq) < self.max_len
            summarytext = np.array(summarytext)
            current_item = {"text": text, "text_seq": text_seq,"sentence_splits": sentence_splits[1:], "sentence_parses": sentence_parses, "summary": summarytext}
    ...
    def __getitem__(self, idx):
        ...
        text_seq = self._lines[idx]["text_seq"]
        # print(text)
        summarytext = self._lines[idx]["summary"]
        # print(summarytext)
        src_ = torch.LongTensor([self.bos_id] + text_seq.tolist())
        tgt_ = torch.LongTensor(text_seq.tolist() + [self.pad_id])
        ...
        return {"text": text, "attn_mask": mask, "sentence_splits": self._lines[idx]["sentence_splits"], "sentence_parses": self._lines[idx]["sentence_parses"], "summary": summarytext}
