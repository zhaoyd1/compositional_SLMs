...
class LazyLoader(object):
    ...
    def __init__(self, path, data_type='data', is_array=False, sent_level=False, transformer_grammar=False, array_data_type=np.int32, skip_size=0):
        ...
        self.tg = transformer_grammar
        # parse_datapath = os.path.join(lazypath, data_type + '.parse')
        if not transformer_grammar:
        ...
        transformed_lenpath = os.path.join(lazypath, data_type + '.transformed_len.pkl')
        lens = pkl.load(open(lenpath, 'rb'))
        if transformer_grammar:
            transformed_lens = pkl.load(open(transformed_lenpath, 'rb'))

        if not sent_level and transformer_grammar:
            docs = []
            splits = []
            current_size = 0
            transformed_current_size = 0
            local_splits = []
            transformed_splits = []
            transformed_docs = []
            local_transformed_splits = []
            for sent_len, transformed_sent_len in zip(lens, transformed_lens):
                if sent_len == 0:
                    # split doc
                    docs.append(current_size)
                    transformed_docs.append(transformed_current_size)
                    splits.append(tuple(local_splits))
                    transformed_splits.append(tuple(local_transformed_splits))
                    local_splits = []
                    local_transformed_splits = []
                    current_size = 0
                    transformed_current_size = 0
                    ...
                    transformed_current_size += transformed_sent_len
                    local_splits.append(current_size)
                    local_transformed_splits.append(transformed_current_size)
            ...
            if current_size > 0:
                ...
                transformed_splits.append(local_transformed_splits)

            self.lens = docs
            self.transformed_lens = transformed_docs
            self.splits = splits
            self.transformed_splits = transformed_splits
    ...
    def __getitem__(self, index):
        ...
        if not isinstance(index, slice):
            ...
            if self.tg:
                transformed_splits = self.transformed_splits[index]
            else:
            ...
            if self.tg:
                ...
                parses = self.parses[index]
        if self.tg:
            return rtn, splits, transformed_splits
        else:
            return rtn, splits, parses
