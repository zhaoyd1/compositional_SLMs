...
class LazyLoader(object):
    ...
    def __init__(self, path, data_type='data', is_array=False, sent_level=False, transformer_grammar=False, array_data_type=np.int32, skip_size=0):
        lazypath = get_lazy_path(path)
        datapath = os.path.join(lazypath, data_type)
        # parse_datapath = os.path.join(lazypath, data_type + '.parse')
        parse_datapath = os.path.join(lazypath, data_type + '.parses.txt')
        self._file_parse = open(parse_datapath, 'r').readlines()
        self.file_parse = self._file_parse
        # get file where array entries are concatenated into one big string
        self._file = open(datapath, 'rb')
        self._seq_file = open(datapath + "_seq", 'rb')
        self.file = self._file
        self.seq_file = self._seq_file
        self.is_array = is_array
        self.array_data_type = array_data_type
        self.total_tokens = 0
        ...
        seq_lenpath = os.path.join(lazypath, data_type + '.seq.len.pkl')
        seq_lens = pkl.load(open(seq_lenpath, 'rb'))
        assert len(lens) == len(seq_lens)
        if not sent_level:
            docs = []
            seq_docs = []
            splits = []
            ...
            local_seq_splits = []
            local_parses = []
            for sent_len, seq_len in zip(lens, seq_lens):
                if sent_len == 0:
                    # split doc
                    assert seq_len == 0
                    docs.append(current_size)
                    seq_docs.append(current_seq_size)
                    parses.append(local_parses)
                    splits.append(tuple(local_splits))
                    seq_splits.append(tuple(local_seq_splits))
                    local_splits = []
                    local_seq_splits = []
                    local_parses = []
                    current_size = 0
                    ...
                    current_seq_size += seq_len
                    cur_parse = eval(self.file_parse[parse_cnt].strip())
                    local_splits.append(current_size)
                    local_seq_splits.append(current_seq_size)
                    local_parses.append(cur_parse)
                    parse_cnt += 1
            ...
            if current_size > 0:
                ...
                seq_splits.append(local_seq_splits)
                parses.append(local_parses)

            self.lens = docs
            self.seq_lens = seq_docs
            self.splits = splits
            self.seq_splits = seq_splits
            self.parses = parses
            self.ends = list(accumulate(self.lens))
            self.seq_ends = list(accumulate(self.seq_lens))
            self.total_tokens = self.ends[-1]
            self.total_seq_tokens = self.seq_ends[-1]
        ...
        if self.ends[-1] == 0:
            ...
            self.seq_file = np.array([], dtype=array_data_type)
        else:
            self.file = np.memmap(self.file, dtype=array_data_type, mode='r', order='C')
            self.seq_file = np.memmap(self.seq_file, dtype=array_data_type, mode='r', order='C')
        self.read_lock = Lock()
    ...
    def __getitem__(self, index):
        ...
        if not isinstance(index, slice):
            if index == 0:
                ...
                seq_start = self.seq_ends[index - 1]
            end = self.ends[index]
            seq_end = self.seq_ends[index]
            rtn = self.file_read(start, end)
            seq_rtn = self.seq_file_read(seq_start, seq_end)
            splits = self.splits[index]
            seq_splits = self.seq_splits[index]
            parses = self.parses[index]
        else:
            # if slice, fetch strings with 1 diskread and then splice in memory
            chr_lens = self.ends[index]
            seq_chr_lens = self.seq_ends[index]
            if index.start == 0 or index.start is None:
                start = 0
                seq_start = 0
            else:
                start = self.ends[index.start - 1]
                seq_start = self.seq_ends[index.start - 1]
            stop = chr_lens[-1]
            seq_stop = seq_chr_lens[-1]
            strings = self.file_read(start, stop)
            seq_strings = self.seq_file_read(seq_start, seq_stop)
            rtn = split_strings(strings, start, chr_lens)
            seq_rtn = split_strings(seq_strings, seq_start, seq_chr_lens)
            splits = self.splits[index]
            seq_splits = self.seq_splits[index]
            parses = self.parses[index]
        
        return rtn, seq_rtn, splits, seq_splits, parses
    ...
    def seq_file_read(self, start=0, end=None):
        """read specified portion of file"""
        data_type_size = np.dtype(self.array_data_type).itemsize
        # atomic reads to avoid
        self.read_lock.acquire()

        rtn = self.seq_file[start:end]
        if self.is_array:
            rtn = rtn.copy()
        self.read_lock.release()
        return rtn
