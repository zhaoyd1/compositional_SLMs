...
class LazyLoader(object):
    ...
    def __init__(self, path, data_type='data', is_array=False, sent_level=False, transformer_grammar=False, array_data_type=np.int32, skip_size=0):
        ...
        span_datapath = os.path.join(lazypath, data_type + '.spans.txt')
        self._file_span = open(span_datapath, 'r').readlines()
        self.file_span = self._file_span
        ...
        if not sent_level:
            ...
            span_cnt = 0
            current_size = 0
            current_transformed_size = 0
            local_splits = []
            local_spans = []
            local_transformed_splits = []
            for sent_len, transformed_len in zip(lens, transformed_lens):
                if sent_len == 0:
                    # split doc
                    docs.append(current_size)
                    spans.append(local_spans)
                    ...
                    current_transformed_size += transformed_len
                    cur_span = eval(self.file_span[span_cnt].strip())
                    local_splits.append(current_size)
                    local_transformed_splits.append(current_transformed_size)
                    local_spans.append(cur_span)
                    span_cnt += 1
    ...
    def __getitem__(self, index):
        ...
        return rtn, splits, transformed_splits, spans
