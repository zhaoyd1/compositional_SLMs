...
from reader.lazy_loader_0001 import LazyLoader
from reader.dataset_0001 import GPT2Dataset
from torch.utils.data.distributed import DistributedSampler
...
class Trainer(object):
                ...
                if (seq_len > 150 and seq_num > 20) or (seq_len > 100 and seq_num > 50):
                    for j in range(0, seq_num, 25):
                        ...
                        for result in results:
                            src_p, tgt_p, mask, relpos = result
    ...
    #     # 取top 300
    #     pass
    def evaluate_doc(self, text_data, seq_data, parses):
        with open(text_data, 'r') as f:
            text_data = f.readlines()
        ...
        with open(seq_data, 'r') as f:
            ...
            for line in tqdm(seq_data, desc='seq'):
                if line.startswith('Sentence'):
                    if len(cur_seq) > 0:
                        all_seq.append(cur_seq)
                        cur_seq = []
                    continue
                cur_seq.append(self.tokenizer.convert_tokens_to_ids(line.strip().split()))
        if len(cur_seq) > 0:
            all_seq.append(cur_seq)
        ...
        with torch.no_grad():
            for i in tqdm(range(len(text_data))):
                ...
                if cur_prefix != []:
                    ...
                    cur_seq = [cur_prefix_seq + seq for seq in cur_seq]
                ...
                if sent_len > 50 and sent_num > 50:
                    for j in range(0, sent_num, 50):
                        tmp_cur_parses = cur_parses[j:j+50]
                        tmp_cur_seq = cur_seq[j:j+50]
                        tmp_cur_text = cur_text
                        inputs = {"chunk_input_ids": None, "chunk_masks": None, "input_ids": None,
                        "masks": None, "group_ids": None, "chunk_parses": None, "chunk_attn_masks": None}
                        ...
                        with ProcessPoolExecutor(max_workers=8) as executor:
                            results = executor.map(process_seq, tmp_cur_seq, [sent_len] * len(tmp_cur_seq))
                        attn_masks = []
                        for result in results:
                            mask = result
                            attn_masks.append(mask)
                        chunk_attn_masks = torch.tensor(np.array(attn_masks), dtype=torch.long).to(self.device)
                        inputs['chunk_input_ids'] = chunk_input_ids
                        inputs['chunk_masks'] = chunk_masks
                        inputs['input_ids'] = input_ids
                        inputs['masks'] = masks
                        inputs['group_ids'] = group_ids
                        inputs['chunk_parses'] = chunk_parses
                        inputs['chunk_attn_masks'] = chunk_attn_masks
                        with torch.cuda.amp.autocast(dtype=amp_dtype):
                            result = self.model(**inputs, ppl=True)
                    ...
                    with ProcessPoolExecutor(max_workers=8) as executor:
                        results = executor.map(process_seq, cur_seq, [sent_len] * len(cur_seq))
                    attn_masks = []
                    for result in results:
                        mask = result
                        attn_masks.append(mask)
                ...
                cur_prefix_seq = cur_seq[min_idx]
                logger.info(_scalar(-torch.logsumexp(-cur_loss, dim=0)) / cur_sent_len)
                cnt += 1
...
if __name__ == '__main__':
    ...
    if args.tg_enabled == 1:
        ...
        mask_rules = masking_utils.get_masking_rules(
            "stack_compose_double_closing_nt", 
                sequence_length=args.max_seq_len * 2,
                memory_length=args.max_seq_len * 2,
    ...
    dataset = GPT2Dataset(lazy_loader, num_samples=args.num_samples, tg=bool(args.tg_enabled), 
                          bos_id=bos_id, ranges=ranges, mask_rules=mask_rules, max_seq_len=args.max_seq_len)
    print(f'total samples: {args.num_samples}')
    
    collator = DefaultCollator()
    collator_fn = collator.generative_r2d2_collate_fn_0001
    ...
    def process_seq(seq, seq_len):
        ...
        chunks = [types.Chunk(None, *chunk) for chunk in chunks]
        chunk = chunks[0]
        mask = chunk.attn_mask[:seq_len * 2, :seq_len * 2]
        for k in range(len(mask)):
            mask[k, k] = 1
    ...
    if args.eval_mode == 2 and is_master:
        trainer.evaluate_doc('corpus/bllip/bllip_test.raw', 'corpus/bllip/gpst_internal_eval_topk.txt', 'corpus/bllip/supar_eval_parses_new.txt')
