...
from reader.lazy_loader_0010 import LazyLoader
from reader.dataset_0010 import GPT2Dataset
from torch.utils.data.distributed import DistributedSampler
...
from masking_0010 import utils as masking_utils
from masking_0010 import masking_types as types
...
class Trainer(object):
    ...
    def train(self, 
              ...
              left=0,
              opening_nt_start_idx=None, 
              closing_nt_start_idx=None):
        ...
        for step, inputs in enumerate(epoch_iterator):
            ...
            with model.no_sync():
                with torch.cuda.amp.autocast(dtype=amp_dtype):
                    result = self.model(**inputs, opening_nt_start_idx=opening_nt_start_idx, closing_nt_start_idx=closing_nt_start_idx, coeff=coeff, temperature=temperature)
    ...
    def train_tg(self, 
        ...
        for step, inputs in enumerate(epoch_iterator):
            ...
            with model.no_sync():
                ...
                # if (step + 1) % accumulation_steps != 0:
                # WeightedSumFunc.a_ij_require_grad = False
                scaler.scale((result.non_struct_loss + result.pointer_loss) / accumulation_steps).backward()
            ...
            if step % log_steps == 0 and step > 0:
                self.logger.info(f'progress:{step}/{total_step} coeff: {coeff} loss: {_scalar(result.non_struct_loss)} pointer loss: {_scalar(result.pointer_loss)} gpt loss: {_scalar(result.gpt_loss)} ' + \
                    f'action loss: {_scalar(result.action_loss)}')
                # with torch.no_grad():
                #     # output generated binary tree
                #     if self.is_master and result.splits is not None:
    ...
    def evaluate_tg_doc(self, text_data, tg_seq_data, tg_spans_data, ranges, mask_rules):
        ...
        with open(tg_seq_data, 'r') as f:
            ...
            for i in tqdm(range(len(tg_seq_data))):
                ...
                if line.strip() not in cur_seq_set:
                    ...
                    seq = self.tokenizer.convert_tokens_to_ids(line)
                    cur_seq.append(seq)
        ...
        cur_span = []
        cur_span_set = set()
        with open(tg_spans_data, 'r') as f:
            tg_spans_data = f.readlines()
            for i in range(len(tg_spans_data)):
                line = tg_spans_data[i].strip()
                if line.startswith('Sentence'):
                    if len(cur_span) > 0:
                        all_tg_span.append(cur_span[:300])
                        cur_span = []
                        cur_span_set = set()
                    continue
                if len(cur_span) >= 300:
                ...
                if line.strip() not in cur_span_set:
                    cur_span_set.add(line.strip())
                    cur_span.append(eval(line.strip()))
            
            if len(cur_span) > 0:
                all_tg_span.append(cur_span)
        ...
        def update_span(cur_span, cur_prefix_span, cur_offset):
            new_spans = []
            for span in cur_span:
                new_span = {}
                for key, value in cur_prefix_span.items():
                    new_span[key] = []
                    for span_item in value:
                        tmp_span = []
                        for j in range(len(span_item)):
                            tmp_span.append(span_item[j])
                        new_span[key].append(tmp_span)
                for key, value in span.items():
                    if key not in new_span:
                        new_span[key] = []
                    for span_item in value:
                        tmp_span = []
                        for j in range(len(span_item)):
                            tmp_span.append(span_item[j] + cur_offset)
                        new_span[key].append(tmp_span)
        ...
        with torch.no_grad():
            for i in tqdm(range(len(text_data))):
                ...
                # cur_seq = [[50289, 50289, 383, 50315, 12036, 50315, 326]]
                cur_span = all_tg_span[cnt]
                # cur_span = [{1:[[0]], 2:[[1,2]]}]
                ...
                if cur_prefix != []:
                    ...
                    cur_offset = len(cur_prefix) - sum([1 for t in cur_prefix if (t >= ranges.opening_non_terminals[0]) and (t < ranges.opening_non_terminals[1])])
                cur_span = update_span(cur_span, cur_prefix_span, cur_offset) # batch * spans
                extra_lengths = [sum([1 for t in seq if (t >= ranges.opening_non_terminals[0]) and (t < ranges.opening_non_terminals[1])]) for seq in cur_seq]
                seq_len = max([len(seq) for seq, extra_length in zip(cur_seq, extra_lengths)])
                ...
                if (seq_len > 150 and seq_num > 20) or (seq_len > 100 and seq_num > 50):
                    for j in range(0, seq_num, 25):
                        ...
                        for result in results:
                            src_p, tgt_p, mask, relpos, pointer_tgt_pos, pointer_mask, positions = result
                            chunk_input_ids.append(src_p)
                            chunk_tgt_ids.append(tgt_p)
                            chunk_masks.append(mask)
                            pointer_tgt_poses.append(pointer_tgt_pos)
                            pointer_masks.append(pointer_mask)
                            position_ids.append(positions)
                        
                        offset = 1
                        src_len = len(chunk_input_ids[0])
                        all_merged_spans = {}
                        for span in tmp_cur_span:
                            for key, value in span.items():
                                ...
                                for span_item in value:
                                    ...
                                    for i in range(len(span_item)):
                                        tmp_span.append(span_item[i] + offset) # avoid changing the original span
                                    all_merged_spans[key].append(tmp_span)
                            offset += src_len
                        ...
                        for max_len, (key, value) in zip(all_max_lens, all_merged_spans.items()):
                            all_merged_spans_tensor[key] = torch.tensor([span + [-100] * (max_len - len(span)) for span in value], dtype=torch.long).to(self.device) 
                        ...
                        inputs['chunk_attn_masks'] = torch.tensor(np.array(chunk_masks), dtype=torch.bool).to(self.device)
                        inputs['pointer_tgt_pos'] = torch.tensor(np.array(pointer_tgt_poses), dtype=torch.long).to(self.device)
                        inputs['pointer_mask'] = torch.tensor(np.array(pointer_masks), dtype=torch.bool).to(self.device)
                        inputs['position_ids'] = torch.tensor(np.array(position_ids), dtype=torch.long).to(self.device)
                        inputs['all_merged_spans'] = all_merged_spans_tensor
                        inputs['opening_nt_start_idx'] = ranges.opening_non_terminals[0]
                        inputs['closing_nt_start_idx'] = ranges.closing_non_terminals[0]
                        with model.no_sync():
                            with torch.cuda.amp.autocast(dtype=torch.float16):
                                result = self.model(**inputs, ppl=True)
                        if cur_loss is None:
                            cur_loss = result.non_struct_loss.sum(-1).detach().cpu()
                        else:
                            cur_loss = torch.cat([cur_loss, result.non_struct_loss.sum(-1).detach().cpu()], dim=0)
                else:
                    inputs = {"chunk_input_ids": None, "chunk_tgt_ids": None}
                    chunk_input_ids = []
                    chunk_tgt_ids = []
                ...
                cur_prefix_span = cur_span[min_idx]
                del cur_span
    ...
    def evaluate_dev(self, text_data, parse_data):
        ...
        self.model.train()
    # def evaluate_doc_tg(self, text_data):
    #     # 取top 300
    #     pass
        ...
        with torch.no_grad():
            for i in tqdm(range(len(text_data))):
                if text_data[i].strip() == "":
                    ...
                    logger.info("new_doc")
                    # print(cur_doc_loss)
                    # print(final_sent_loss)
                    # logger.info(f'doc loss: {cur_doc_loss}')
                    # logger.info(f'final sent loss: {final_sent_loss}')
                    cur_doc_loss = 0
                    doc_num += 1
                    exclude = False
                    continue
                if exclude:
                    ...
                    continue
                cur_text = self.tokenizer.encode(text_data[i].strip())
                cur_parses = all_parses[cnt]
                cur_sent_len = len(cur_text)
                sent_len_list.append(cur_sent_len)
                ...
                if cur_prefix != []:
                    ...
                    for j in range(len(cur_parses)):
                        for k in range(len(cur_parses[j])):
                            cur_parses[j][k] = cur_parses[j][k] + len(cur_prefix_parse) + 1
                    
                    cur_parses = [[len(cur_prefix_parse)] + cur_prefix_parse + cur_parse for cur_parse in cur_parses]
                sent_len = len(cur_text)
                if sent_len > 2048:
                    logger.info(f'exceed 2048 error: {sent_len}, {cnt}')
                    cnt += 1
                    if doc_num != prev_doc_num:
                        fail_num += 1
                        prev_doc_num = doc_num
                ...
                cur_loss = None
                segment_ids = []
                for j in range(len(sent_len_list)):
                    segment_ids += [j + 1] * sent_len_list[j]
                max_sent_len = max(sent_len_list)
                start_id = 0
                ids_list = []
                for j in range(len(sent_len_list)):
                    end_id = start_id + sent_len_list[j]
                    ids_list.append(cur_text[start_id:end_id] + [0] * (max_sent_len - sent_len_list[j]))
                    start_id = end_id
                
                assert len(segment_ids) == sent_len

                if sent_len > 70 and sent_num > 20 or sent_num > 50:
                    for j in range(0, sent_num, 20):
                        tmp_cur_parses = cur_parses[j:j+20]
                        tmp_cur_text = cur_text
                        inputs = {"chunk_input_ids": None, "chunk_masks": None, "input_ids": None,
                        "masks": None, "group_ids": None, "chunk_parses": None}
                        chunk_input_ids = torch.tensor(tmp_cur_text, dtype=torch.long).repeat(len(tmp_cur_parses), 1).view(-1, sent_len).to(self.device)
                        chunk_masks = torch.tensor(segment_ids, dtype=torch.long).repeat(len(tmp_cur_parses), 1).view(-1, sent_len).to(self.device)
                        input_ids = torch.tensor(ids_list, dtype=torch.long).repeat(len(tmp_cur_parses), 1).to(self.device)
                        masks = torch.where(input_ids != 0, 1, 0).to(self.device)
                        group_ids = np.array([[k] * len(sent_len_list) for k in range(len(tmp_cur_parses))]).reshape(-1)
                        chunk_parses = torch.tensor(tmp_cur_parses, dtype=torch.long).to(self.device)
                        inputs['chunk_input_ids'] = chunk_input_ids
                        inputs['chunk_masks'] = chunk_masks
                        inputs['input_ids'] = input_ids
                        inputs['masks'] = masks
                        ...
                        if cur_loss is None:
                            ...
                            cur_loss = torch.cat([cur_loss, cur_gpt_loss + cur_action_loss], dim=0)
                else:
                    inputs = {"chunk_input_ids": None, "chunk_masks": None, "input_ids": None,
                            "masks": None, "group_ids": None, "chunk_parses": None}
                    chunk_input_ids = torch.tensor(cur_text, dtype=torch.long).repeat(sent_num, 1).view(-1, sent_len).to(self.device)
                    chunk_masks = torch.tensor(segment_ids, dtype=torch.long).repeat(sent_num, 1).view(-1, sent_len).to(self.device)
                    input_ids = torch.tensor(ids_list, dtype=torch.long).repeat(sent_num, 1).to(self.device)
                    masks = torch.where(input_ids != 0, 1, 0).to(self.device)
                    group_ids = np.array([[k] * len(sent_len_list) for k in range(sent_num)]).reshape(-1)
                    ...
                    cur_action_loss = result.action_loss[:, :-1].sum(-1)
                    cur_loss = cur_gpt_loss + cur_action_loss
                final_sent_loss = cur_loss[0].sum().item()
                cur_loss -= cur_prefix_loss
                cur_doc_loss += _scalar(-torch.logsumexp(-cur_loss, dim=0))
                cur_prefix_loss = cur_loss.min().item() + cur_prefix_loss
                min_idx = cur_loss.argmin().item()
                cur_prefix = cur_text
                cur_prefix_parse = cur_parses[min_idx]
                logger.info(_scalar(-torch.logsumexp(-cur_loss, dim=0)) / cur_sent_len)
                cnt += 1
...
if __name__ == '__main__':
    ...
    cmd.add_argument('--model_type', choices=['r2d2-gen', 'gpt', 'llama', 'r2d2', 'r2d2-gen-fast', 'r2d2-fast', 'r2d2-gen-fast-struct', 'r2d2-gen-fast-ext', 'tg', 'n-ary-gen'], default='r2d2-gen')
    ...
    if args.tg_enabled == 1:
        ...
        startofword = [False for _ in range(len(vocab_list.keys()))]
        for key, val in vocab_list.items():
            if 'Ġ' in key:
                startofword[val] = True
        # print(startofword)
        # print(len(startofword))
        # exit()
        ranges = masking_utils.TokenTypeRanges(bos_id, -100, eos_id, opening_nt, closing_nt, startofword)
    ...
    dataset = GPT2Dataset(lazy_loader, num_samples=args.num_samples, tg=bool(args.tg_enabled), 
                          bos_id=bos_id, ranges=ranges, mask_rules=mask_rules, max_seq_len=args.max_seq_len - 1 if args.tg_enabled == 1 else args.max_seq_len)
    print(f'total samples: {args.num_samples}')
    
    collator = DefaultCollator()
    collator_fn = collator.generative_r2d2_collate_fn_0010
    ...
    def process_seq(seq, seq_len):
        ...
        mask = chunk.attn_mask[:seq_len+1, :seq_len+1]
        for k in range(len(mask)):
            mask[k, k] = 1
        chunk_len = len(chunk.inputs)
        relpos = chunk.attn_relpos[:seq_len+1, chunk_len:chunk_len+seq_len+1]
        pointer_tgt_pos = chunk.pointer_tgt_pos[:seq_len+1]
        pointer_mask = chunk.pointer_mask[:seq_len+1, :seq_len+1]
        positions = chunk.position_ids[:seq_len+1]
        return src_p, tgt_p, mask, relpos, pointer_tgt_pos, pointer_mask, positions
    ...
    if args.tg_enabled == 1 and args.eval_mode == 0:
        trainer.train_tg(dataloader, optimizer, scheduler, scaler,
                  ...
                  max_recover_step=max_step, eval_steps=args.eval_steps, dev_path=args.dev_path, ranges=ranges, mask_rules=mask_rules, opening_nt_start_idx=ranges.opening_non_terminals[0], closing_nt_start_idx=ranges.closing_non_terminals[0])
    elif args.tg_enabled == 1 and args.eval_mode == 2 and is_master:
        trainer.evaluate_tg_doc('corpus/bllip/bllip_test.raw', 'corpus/bllip/0010_topk.txt', 'corpus/bllip/0010_topk_spans.txt', ranges, mask_rules)
        ...
        trainer.train_sent(text_data, parses, optimizer, scheduler, scaler,
                           ...
                           log_steps=args.log_steps, save_steps=args.save_steps,
                           accumulation_steps=args.accumulation_steps,
                           eval_steps=args.eval_steps, dev_path=args.dev_path, left=args.left, opening_nt_start_idx=ranges.opening_non_terminals[0], closing_nt_start_idx=ranges.closing_non_terminals[0])
    else:
        trainer.train(dataloader, optimizer, scheduler, scaler,
                  ...
                  log_steps=args.log_steps, save_steps=args.save_steps,
                  accumulation_steps=args.accumulation_steps,
                  max_recover_step=max_step, eval_steps=args.eval_steps, dev_path=args.dev_path, left=args.left, opening_nt_start_idx=ranges.opening_non_terminals[0], closing_nt_start_idx=ranges.closing_non_terminals[0])