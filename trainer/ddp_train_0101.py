...
from reader.dataset_0101 import GPT2Dataset
from torch.utils.data.distributed import DistributedSampler
...
from masking_0101 import utils as masking_utils
from masking_0101 import masking_types as types
...
class Trainer(object):
    ...
    def evaluate_doc(self, text_data, gpst_seq_data, ranges, mask_rules):
        ...
        all_gpst_seq = []
        cur_seq = []
        with open(gpst_seq_data, 'r') as f:
            gpst_seq_data = f.readlines()
            for i in range(len(gpst_seq_data)):
                line = gpst_seq_data[i].strip()
                if line.startswith('Sentence'):
                    if len(cur_seq) > 0:
                        all_gpst_seq.append(cur_seq[:300])
                        cur_seq = []
                    continue
                seq = self.tokenizer.convert_tokens_to_ids(line.split())
                cur_seq.append(seq)
            if len(cur_seq) > 0:
                all_gpst_seq.append(cur_seq)
        ...
        with torch.no_grad():
            for i in tqdm(range(len(text_data))):
                ...
                # print(cur_doc_loss.dtype)
                cur_prefix_loss = cur_loss.min().item() + cur_prefix_loss
                # print(cur_prefix_loss.dtype)
                # exit()
