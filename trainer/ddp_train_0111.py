...
from reader.dataset_0111 import GPT2Dataset
from torch.utils.data.distributed import DistributedSampler
...
from masking_0111 import utils as masking_utils
from masking_0111 import masking_types as types
...
class Trainer(object):
    ...
    def train_tg(self, 
        ...
        for step, inputs in enumerate(epoch_iterator):
            ...
            if step % log_steps == 0 and step > 0:
                self.logger.info(f'progress:{step}/{total_step} coeff: {coeff} loss: {_scalar(result.non_struct_loss)} pointer loss: {_scalar(result.pointer_loss)}')
                # with torch.no_grad():
                #     # output generated binary tree
                #     if self.is_master and result.splits is not None:
                #         # output binary trees for different iteration epochs
...
if __name__ == '__main__':
    ...
    collator_fn = collator.generative_r2d2_collate_fn_0111
