...
from reader.dataset_1010 import GPT2Dataset
from torch.utils.data.distributed import DistributedSampler
...
from masking_1010 import utils as masking_utils
from masking_1010 import masking_types as types
    ...
    def evaluate_tg_doc(self, text_data, tg_seq_data, ranges, mask_rules):
        ...
        with torch.no_grad():
            for i in tqdm(range(len(text_data))):
                ...
                extra_lengths = [sum([1 for t in seq if (t >= ranges.closing_non_terminals[0]) and (t < ranges.closing_non_terminals[1])]) for seq in cur_seq]
                seq_len = max([len(seq) + extra_length for seq, extra_length in zip(cur_seq, extra_lengths)])
                ...
                if (seq_len > 150 and seq_num > 20) or (seq_len > 100 and seq_num > 50):
                    for j in range(0, seq_num, 25):
                        ...
                        inputs = {"chunk_input_ids": None, "chunk_tgt_ids": None, "attn_masks": None, "rel_positions": None}
                        
                        with ProcessPoolExecutor(max_workers=8) as executor:
                            results = executor.map(process_seq, tmp_cur_seq, [seq_len] * len(tmp_cur_seq))

                        for result in results:
                            src_p, tgt_p, mask, relpos, pointer_tgt_pos, pointer_mask = result
                        ...
                        inputs['attn_masks'] = torch.tensor(np.array(chunk_masks), dtype=torch.bool).to(self.device)
                        inputs['rel_positions'] = torch.tensor(np.array(rel_positions), dtype=torch.long).to(self.device)
...
if __name__ == '__main__':
    ...
    model = create_model(args.model_type, args.r2d2_config_path, args.gpt_config_path, args.fix_embedding, args.gradient_checkpoint, opening_included=True)
    ...
    collator_fn = collator.generative_r2d2_collate_fn_1010
    ...
    def process_seq(seq, seq_len):
        ...
        return src_p, tgt_p, mask, relpos, pointer_tgt_pos, pointer_mask
    ...
    if args.tg_enabled == 1 and args.eval_mode == 0:
        ...
        trainer.evaluate_tg_doc('corpus/bllip/bllip_test.raw', 'corpus/bllip/tg_nonlabel_test_doc_level_parses.txt', ranges, mask_rules)
