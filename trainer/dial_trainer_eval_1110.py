...
from reader.dataset_dialog_1110 import XSumDataset
...
from utils.beam_searcher_1110_gen import R2D2GenFastBeamSearcher
...
if __name__ == '__main__':
    ...
    if args.pretrain_dir is not None:
        if args.model_type == 'tg':
            if is_master:
                state_dicts = torch.load(os.path.join(args.pretrain_dir), map_location=lambda a, b: a)
                out_dict = {}
                for key, val in state_dicts.items():
    ...
    collator_fn = collator.xsum_collate_fn_1111
    ...
    if is_master and args.eval_perepoch:
        ...
        eval_dataset = XSumDataset(data_dir=args.summary_dir, mode="test", tokenizer=tokenizer, eos_id=eos_id, document_threshold=args.document_threshold, summary_threshold=args.summary_threshold,
                                   max_len=args.max_seq_len, ranges=ranges, maskrules=mask_rules)
        eval_collator_fn = XSumCollator("test", padding=-1).xsum_collate_fn_1111
        ...
        eval_model = model
        generator = R2D2GenFastBeamSearcher(eval_model, gpt_config, device, beam_size=args.beam_size, sampling=True)
