...
from model.model_factory import create_model, glue_create_model
from reader.data_collator import DefaultCollator, GlueCollator
from reader.dataset_glue import GlueDataset
...
class Trainer(object):
    ...
    def train(self, 
              ...
              cls_id=50259, min_label_id=50260, 
        ...
        for epoch in train_iterator:
            ...
            # coeff = 1.0 if epoch < args.epochs // 2 else 0.0
            # temperature = 1.0 if epoch < args.epochs // 2 else 0.1
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.model.no_sync():
                    with torch.cuda.amp.autocast(dtype=amp_dtype):
                        result = self.model(**inputs, coeff=coeff, temperature=temperature, cls_id=cls_id, min_label_id=min_label_id)
                ...
                if curr_step % log_steps == 0 and curr_step > 0:
                    self.logger.info(f'progress:{curr_step}/{total_step} coeff: {coeff} temperature: {temperature} loss: {_scalar(result.non_struct_loss + result.struct_loss)} gpt loss: {_scalar(result.gpt_loss)} ' + \
                        ...
                        f'glue finetune loss: {_scalar(result.glue_finetune_loss)}')
                    with torch.no_grad():
            ...
            if self.is_master and evaluator is not None:    
                score = evaluator.eval(eval_data_loader, temperature=1.0)
                self.model.train()
...
if __name__ == '__main__':
    ...
    cmd.add_argument('--task_name', default="sst-2", type=str, help='task name of glue')
    cmd.add_argument('--glue_dir', required=True, type=str, help="directory of the glue task data")
    ...
    cmd.add_argument('--finetune_type', choices=['generative', 'discriminant'], default='generative')
    ...
    if args.finetune_type == "generative":
        model = glue_create_model(args.model_type, args.r2d2_config_path, args.gpt_config_path, args.fix_embedding, args.gradient_checkpoint, finetune_class_num=-1)
    else:
        model = glue_create_model(args.model_type, args.r2d2_config_path, args.gpt_config_path, args.fix_embedding, args.gradient_checkpoint, finetune_class_num=TASK_CLASS_NUM[args.task_name])
    ...
    dataset = GlueDataset(task_name=args.task_name, data_dir=args.glue_dir, mode="train", tokenizer=tokenizer)
    cls_ids_list = clstgt_ids[0:len(dataset.labels)]
    logger.info(f'finished loading dataset on {global_rank}')
    
    collator = GlueCollator(clstgt_ids, not args.disable_group, \
        external_vocab_path=args.ext_vocab_path, padding=-1)
    collator_fn = collator.generative_r2d2_glue_collate_fn
    ...
    if is_master and args.eval_perepoch:
        ...
        eval_collator_fn = GlueCollator(clstgt_ids, not args.disable_group, \
    ...
    trainer.train(dataloader, eval_dataloader, optimizer, scheduler, scaler, evaluator, 
                  ...
                  cls_id=cls_id, min_label_id=min_label_id, 
