        ...
        for step, inputs in enumerate(epoch_iterator):
            ...
            with model.no_sync():
                with torch.cuda.amp.autocast(dtype=amp_dtype):
                    result = self.model(**inputs, temperature=temperature)
            ...
            if step % log_steps == 0 and step > 0:
                self.logger.info(f'progress:{step}/{total_step} loss: {_scalar(result.non_struct_loss + result.struct_loss)} gpt loss: {_scalar(result.gpt_loss)} ' + \
                    f'inside_outside loss: {_scalar(result.inside_outside_loss)} action loss: {_scalar(result.action_loss)}')
                # with torch.no_grad():
...
if __name__ == '__main__':
    ...
    collator_fn = collator.generative_r2d2_collate_fn
    ...
    if global_rank == -1:
        ...
        optimizer = AdamW(model_params, lr=args.lr, correct_bias=False)
