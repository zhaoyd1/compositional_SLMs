...
from reader.dataset_xsum_0110 import XSumDataset
...
from utils.beam_searcher_0110_gen import R2D2GenFastBeamSearcher
...
if __name__ == '__main__':
    ...
    cmd.add_argument('--max_seq_len', default=1024, type=int, help="max sequence length")
    cmd.add_argument('--tg_enabled', default=1, type=int, help="enable tg")
    ...
    dataset = XSumDataset(data_dir=args.summary_dir, mode="test", tokenizer=tokenizer, eos_id=eos_id, document_threshold=args.document_threshold, summary_threshold=args.summary_threshold, 
                          max_len=args.max_seq_len, ranges=ranges, maskrules=mask_rules)
    logger.info(f'finished loading dataset on {global_rank}')
    
    collator = XSumCollator("test", padding=-1)
    collator_fn = collator.xsum_collate_fn_0110
    ...
    if is_master and args.eval_perepoch:
        ...
        eval_dataset = XSumDataset(data_dir=args.summary_dir, mode="test", tokenizer=tokenizer, eos_id=eos_id, document_threshold=args.document_threshold, summary_threshold=args.summary_threshold,
                                   max_len=args.max_seq_len, ranges=ranges, maskrules=mask_rules)
        eval_collator_fn = XSumCollator("test", padding=-1).xsum_collate_fn_0110
        ...
        eval_model = model
        generator = R2D2GenFastBeamSearcher(eval_model, gpt_config, device, beam_size=args.beam_size, sampling=True, startofword=startofword)
