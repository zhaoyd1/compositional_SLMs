...
from model.stack_state_0000_gen import ActionType, Node, State, BeamContext
import numpy as np
import torch
...
class R2D2GenFastBeamSearcher:
    def __init__(self, model, config, device, beam_size=10, sampling=False, **kwargs):
        self.model = model
        model.to(device)
        self.model.eval()
    ...
    def beam_search(self, 
        ...
        if chunk_input_ids is not None:
            ...
            action_len_bucket, token_len_bucket = beam_context.init_history_kv(past_action_kvs, seq_lens, group_ids)
        ...
        while True:
            ...
            # update context
            beam_context.update(states_batch, sync_steps, reduce_repr, last_action_kv)
