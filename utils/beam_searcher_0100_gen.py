...
from model.stack_state_0100_gen import ActionType, State, BeamContext
import numpy as np
import torch
...
class R2D2GenFastBeamSearcher:
    def __init__(self, model, config, device, beam_size=10, opening_nt=50289, closing_nt=50315, fast_shift_size=5, sampling=False):
        self.model = model
        model.to(device)
        self.model.eval()
    ...
    def next_actions(self, action_logits, active_states, sync_steps, input_ids=None, atom_spans=None):
        ...
        token_slice = list(range(0, self.closing_nt)) + list(range(self.closing_nt + 1, action_space))
        ...
        for batch_i, indices in enumerate(top_indices):
            ...
            for score_idx, sorted_idx in enumerate(indices):
                ...
                if top_scores[batch_i, score_idx] > float('-inf'):
                    ...
                    if action_id == self.closing_nt and active_states[batch_i][beam_id].current_action == ActionType.REDUCE:
    ...
    def beam_search(self, 
        ...
        states_batch = []
        max_action_steps = 3 * max_steps + 3
