...
from model.stack_state_0110_gen import ActionType, State, BeamContext
import numpy as np
import torch
...
class R2D2GenFastBeamSearcher:
    def __init__(self, model, config, device, beam_size=10, opening_nt=50289, closing_nt=50315, fast_shift_size=5, startofword=None, sampling=False):
        self.model = model
        model.to(device)
        self.model.eval()
        self.gpt_config = config
        self.beam_size = beam_size
        self.fast_shift_size = 0
        ...
        self.opening_nt = opening_nt
        self.closing_nt = closing_nt
    ...
    def step(self, states_batch, beam_context, sync_steps):
        ...
        if gpt_input is not None:
            ...
            # gpt_input is already in the shape of (N, 1)
            action_result = self.model.gpt(input_ids=gpt_input, position_ids=position_ids, past_key_values=action_kv, attention_mask=action_masks, return_dict=True)
            # action_logits = self.model.action_mlp(action_result.last_hidden_state)  # (N * B, 1, 2)
            action_logits = action_result.logits
            ...
            pointer_network_cache_last_slice = current_pointer_network_cache # (N * B, 1, n_dim // 6)
            return action_logits, pointer_logits, action_kv_last_slice, pointer_network_cache_last_slice
        else:
    ...
    def bos_step(self, N):
        ...
        gpt_input = self.bos_id * torch.ones((N, 1), dtype=torch.long, device=self.device)
        action_result = self.model.gpt(input_ids=gpt_input, return_dict=True)
        action_logits = action_result.logits
        ...
        return action_logits, pointer_logits, action_result.past_key_values, pointer_network_cache
        
    def next_actions(self, action_logits, pointer_logits, active_states, sync_steps, input_ids=None, atom_spans=None):
        # action_logits: (?, 1, vocab_size); vocab_size includes REDUCE, SHIFT
        action_logits = action_logits.squeeze(1)
        pointer_logits = pointer_logits.squeeze(1) # (?, L + 1)
        flatten_states = []
        # padding action_logits
        max_beam_size = 0
        score_masks = []
        gather_indices = []
        token_ids = []
        state_idx = 0
        base_scores = []
        pointer_extra_mask = [] # to avoid unary chain
        ...
        for batch_i, states in enumerate(active_states):
            for state_i, state in enumerate(states):
                ...
                if state.token_offset == sync_steps[batch_i] and not state.is_finished:
                    ...
                    #     pointer_extra_mask.append((state_idx, state.cur_limited_pointer_candidate_pos))
        ...
        action_space = action_logits.shape[-1]
        if not torch.all(pointer_logits == float('-inf')):
            pointer_logits = F.log_softmax(pointer_logits, dim=-1) # (?, L + 1)
        pointer_extra_mask = torch.tensor(pointer_extra_mask, device=self.device)
        ...
        # pointer_scores = torch.where(pointer_scores == float('-inf'), pointer_scores, 0)
        ...
        gather_indices = torch.tensor(gather_indices, device=self.device) # (?, 2)
        ...
        token_slice = list(range(0, self.closing_nt - 1)) + list(range(self.closing_nt + 1, action_space)) # borrow self.closing_nt - 1 for top 2 pointer scores
        action_scores = F.log_softmax(action_logits, dim=-1) # (?, vocab_size)
        token_scores = action_scores[:, token_slice]  # (?, token_vocab_size)
        # choose the target ids and mask out other tokens, if there is any target ids
        if input_ids is not None:
            token_masks = torch.zeros_like(token_scores).fill_(False).bool()
            token_ids = torch.tensor(token_ids, dtype=torch.long, device=self.device) # (?, 2)
            token_masks[token_ids[:, 0], token_ids[:, 1]] = True
            token_scores = token_scores.masked_fill(~token_masks, float('-inf'))
        # add token mask by stack state contraints
        token_masks = score_masks[:, 0]  # (?,)
        token_scores = token_scores.masked_fill(~token_masks.unsqueeze(-1), float('-inf'))
        # mask out opening nts by stack state constraints
        # opening_masks = score_masks[:, 2]  # (?,)
        # action_scores[:, self.opening_nt].masked_fill_(~opening_masks, float('-inf'))
        # mask out closing nts by stack state constraints
        # action_scores[:, self.closing_nt] = float('-inf')
        reduce_masks = score_masks[:, 1]  # (?,)
        reduce2_masks = score_masks[:, 2]  # (?,)
        all_reduce_masks = reduce_masks | reduce2_masks
        action_scores[:, self.closing_nt - 1] = action_scores[:, self.closing_nt] # set the duplicated closing nt score to the same as the original closing nt, for top 2 pointer scores
        action_scores[:, self.closing_nt - 1] += pointer_scores[:, 1] # (?,)
        action_scores[:, self.closing_nt] += pointer_scores[:, 0] # (?,)
        action_scores[:, self.closing_nt].masked_fill_(~all_reduce_masks, float('-inf'))
        action_scores[:, self.closing_nt].masked_fill_(reduce2_masks, 0) # set the duplicated closing nt score to 0 as we do not have to make predictions
        action_scores[:, self.closing_nt - 1].masked_fill_(~all_reduce_masks, float('-inf'))
        action_scores[:, self.closing_nt - 1].masked_fill_(reduce2_masks, 0)
        action_scores[:, self.closing_nt - 1] = float('-inf')
        action_scores[:, token_slice] = token_scores # (?, vocab_size)
        ...
        vocab_size = action_logits.shape[-1]
        # sort
        ...
        for batch_i, indices in enumerate(top_indices):
            ...
            for score_idx, sorted_idx in enumerate(indices):
                beam_id = sorted_idx // vocab_size
                action_id = sorted_idx % vocab_size
                if top_scores[batch_i, score_idx] > float('-inf'):
                    # ensure not invalid action
                    assert active_states[batch_i][beam_id].batch_idx != -1
                    if action_id in [self.closing_nt, self.closing_nt - 1] and active_states[batch_i][beam_id].current_action == ActionType.REDUCE:
                        # breakpoint()
                        state = active_states[batch_i][beam_id].act(ActionType.REDUCE2, token_id=self.closing_nt)
                    elif action_id in [self.closing_nt, self.closing_nt - 1]:
                        # breakpoint()
                        state = active_states[batch_i][beam_id].act(ActionType.REDUCE, token_id=self.closing_nt,
                                                                    pointer_tgt_id=max(pointer_indices[active_states[batch_i][beam_id].batch_idx, self.closing_nt - action_id].item() - self.action_kv_len, 0)) # self.closing_nt - 1 -> [:, 1], self.closing_nt -> [:, 0]
                    else:
    ...
    def update_sync_steps(self, states_batch, sync_steps, marginal_logp=None, force_sync=None):
        ...
        for batch_i, states in enumerate(states_batch):
            ...
            if all_pass:
                if marginal_logp != None:
                    ...
                    # print([state.is_finished for state in states_batch[0]])
                    # print([state.invalid_state for state in states_batch[0]])
                    # breakpoint()
        ...
        return states_batch, sync_steps, marginal_logp
    ...
    def beam_search(self, 
        ...
        states_batch = []
        max_action_steps = 3 * max_steps + 4
        ...
        if chunk_input_ids is not None:
            outputs = self.model(chunk_input_ids=chunk_input_ids, 
                                 chunk_tgt_ids=chunk_tgt_ids, 
                                 attn_masks=attn_masks,
                                 pointer_tgt_pos=pointer_tgt_pos,
                                 pointer_mask=pointer_mask,
                                 past_key_values=past_kv)
            
            past_action_kvs = outputs.past_kv
            pointer_network_cache = outputs.hidden_states[..., :self.gpt_config.n_embd // 6] # (N, L, n_dim // 6)
            seq_lens = (chunk_input_ids != 0).sum(dim=1)
            action_kv_len = past_action_kvs[0][0].shape[2]
            self.action_kv_len = action_kv_len
            pointer_cache_len = pointer_network_cache.shape[1]
            # print("action_kv_len: ", action_kv_len)
            # print("pointer_cache_len: ", pointer_cache_len)
            # assert action_kv_len == pointer_cache_len
            # breakpoint()
            ...
            action_len_bucket = beam_context.init_history_kv(past_action_kvs, pointer_network_cache, seq_lens=seq_lens, attn_masks=attn_masks)
            action_len_bucket_t = torch.tensor(action_len_bucket, device=self.device)
            action_logits = outputs.logits.gather(dim=1, index=(action_len_bucket_t - 1).unsqueeze(1).unsqueeze(2).repeat(1, 1, outputs.logits.shape[-1]))
            pointer_logits = outputs.pointer_logits.gather(dim=1, index=(action_len_bucket_t - 1).unsqueeze(1).unsqueeze(2).repeat(1, 1, outputs.pointer_logits.shape[-1]))
            # breakpoint()
            last_action_kv = None
            last_pointer_network_cache = None
            for batch_i in range(N):
                init_state = State(None)
                init_state.beam_id = 0
                init_state.max_step = max_action_steps
                init_state.total_len = seq_lens_np[batch_i]
                init_state.token_id = chunk_input_ids[batch_i][seq_lens_np[batch_i] - 1]
                init_state.startofword = self.startofword
                states_batch.append([init_state])
        else:
            beam_context = BeamContext(self.model, N, self.beam_size, max_steps, 
                                    self.gpt_config, self.device)
            action_logits, pointer_logits, last_action_kv, last_pointer_network_cache = self.bos_step(N)
        ...
        sync_steps = [0] * N
        while True:
            candidate_states = self.next_actions(action_logits, pointer_logits, states_batch, sync_steps, target_ids, atom_spans)
            # update beam context
            ...
            # update context
            force_sync = beam_context.update(states_batch, sync_steps, last_action_kv, last_pointer_network_cache)

            # for those beams all synchronized on token, pass next token to gpt
            if tag != None:
               states_batch, sync_steps, marginal_logp = self.update_sync_steps(states_batch, sync_steps, marginal_logp, force_sync)
            else:
               states_batch, sync_steps, _ = self.update_sync_steps(states_batch, sync_steps, force_sync=force_sync)                        
            action_logits, pointer_logits, last_action_kv, last_pointer_network_cache = self.step(states_batch, beam_context, sync_steps)
