...
from model.stack_state_1011 import ActionType, State, BeamContext
import numpy as np
import torch
...
class R2D2GenFastBeamSearcher:
    ...
    def step(self, states_batch, beam_context, sync_steps):
        ...
        if gpt_input is not None:
            ...
            return action_logits, token_logits, action_kv_last_slice
        else:
    ...
    def bos_step(self, N):
        ...
        action_logits = self.model.action_mlp(action_result.last_hidden_state) # (N, 1, 3)
        token_logits = self.model.classifier(self.model.dense(action_result.last_hidden_state))
        return action_logits, token_logits, action_result.past_key_values
        
    def next_actions(self, action_logits, token_logits, active_states, sync_steps, input_ids=None, atom_spans=None):
        # action_logits: (?, 1, 3), token_logits(?, 1, vocab_size)
        ...
        action_space = token_logits.shape[-1] + 2  # 2 for shift and reduce
        ...
        # breakpoint()
        action_scores = F.log_softmax(action_logits, dim=-1) # (?, 3)
        ...
        open_scores = action_scores[:, ActionType.OPEN.value]  # (?), probability of opening a nt
        action_scores = torch.cat([gen_scores, compose_scores.unsqueeze(-1), open_scores.unsqueeze(-1)], dim=-1) # (?, vocab_size + 2)
        ...
        for batch_i, indices in enumerate(top_indices):
            ...
            for score_idx, sorted_idx in enumerate(indices):
                ...
                if top_scores[batch_i, score_idx] > float('-inf'):
                    ...
                    if action_id == vocab_size + 1:
                        state = active_states[batch_i][beam_id].act(ActionType.OPEN, token_id=50289)
                    elif action_id == vocab_size:
    ...
    def beam_search(self, 
        ...
        if chunk_input_ids is not None:
            ...
            action_logits, token_logits, last_action_kv = self.bos_step(N)

        for batch_i in range(N):
            init_state = State(None)
            init_state.beam_id = 0
            init_state.max_step = max_steps * 3 + 4
        ...
        while True:
            ...
            # update context
            force_sync = beam_context.update(states_batch, sync_steps, reduce_repr, last_action_kv)
            ...
            if tag != None:
                ...
                sync_steps, _ = self.update_sync_steps(states_batch, sync_steps, force_sync=force_sync)                        
            action_logits, token_logits, last_action_kv = self.step(states_batch, beam_context, sync_steps)
            self.forward_step += 1
