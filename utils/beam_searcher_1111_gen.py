...
from model.stack_state_1111_gen import ActionType, State, BeamContext
import numpy as np
import torch
...
class R2D2GenFastBeamSearcher:
    def __init__(self, model, config, device, beam_size=10, opening_nt=50289, closing_nt=50315, fast_shift_size=1, sampling=False):
        self.model = model
        model.to(device)
        self.model.eval()
    ...
    def step(self, states_batch, beam_context, sync_steps):
        ...
        gpt_input, position_ids, action_kv, action_masks = beam_context.prepare_gpt_input(states_batch, sync_steps)
        if gpt_input is not None:
            ...
            return action_logits, action_kv_last_slice
        else:
            return None, None
    ...
    def bos_step(self, N):
        ...
        action_logits = action_result.logits
        return action_logits, action_result.past_key_values
        
    def next_actions(self, action_logits, active_states, sync_steps, input_ids=None, atom_spans=None):
        # action_logits: (?, 1, vocab_size); vocab_size includes OPEN, REDUCE, SHIFT
        ...
        token_slice = list(range(0, self.opening_nt)) + list(range(self.opening_nt + 1, self.closing_nt)) + list(range(self.closing_nt + 1, action_space))
        ...
        for batch_i, indices in enumerate(top_indices):
            ...
            for score_idx, sorted_idx in enumerate(indices):
                ...
                if top_scores[batch_i, score_idx] > float('-inf'):
                    ...
                    if action_id == self.opening_nt:
                        state = active_states[batch_i][beam_id].act(ActionType.OPEN, token_id=action_id)
                    elif action_id == self.closing_nt and active_states[batch_i][beam_id].current_action == ActionType.REDUCE:
                        state = active_states[batch_i][beam_id].act(ActionType.REDUCE2, token_id=action_id)
                    elif action_id == self.closing_nt:
                        state = active_states[batch_i][beam_id].act(ActionType.REDUCE, token_id=action_id)
                    else:
    ...
    def beam_search(self, 
        ...
        # breakpoint()
        states_batch = []
        max_action_steps = 4 * max_steps + 4
        ...
        if chunk_input_ids is not None:
            ...
            action_len_bucket = beam_context.init_history_kv(past_action_kvs, seq_lens=seq_lens, attn_masks=attn_masks)
            ...
            action_logits, last_action_kv = self.bos_step(N)
        ...
        sync_steps = [0] * N
        while True:
            candidate_states = self.next_actions(action_logits, states_batch, sync_steps, target_ids, atom_spans)
            # update beam context
            ...
            # update context
            force_sync = beam_context.update(states_batch, sync_steps, last_action_kv)
            ...
            # if sync_steps[0] == 100:
            #     breakpoint()
            action_logits, last_action_kv = self.step(states_batch, beam_context, sync_steps)
